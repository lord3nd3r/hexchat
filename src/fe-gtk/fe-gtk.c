...
static const GOptionEntry gopt_entries[] = {
    ...
    {"cfgdir", 'd', 0, G_OPTION_ARG_STRING, &arg_cfgdir,
     N_("Use a different config directory"), "PATH"},
    {"no-plugins", 'n', 0, G_OPTION_ARG_NONE, &arg_skip_plugins,
     N_("Don't auto load any plugins"), NULL},
    {"plugindir", 'p', 0, G_OPTION_ARG_NONE, &arg_show_autoload,
     N_("Show plugin/script auto-load directory"), NULL},
    {"configdir", 'u', 0, G_OPTION_ARG_NONE, &arg_show_config,
     N_("Show user config directory"), NULL},
    {"url", 0, G_OPTION_FLAG_HIDDEN, G_OPTION_ARG_STRING, &arg_url,
     N_("Open an irc://server:port/channel?key URL"), "URL"},
    {"command", 'c', 0, G_OPTION_ARG_STRING, &arg_command,
     N_("Execute command:"), "COMMAND"},
#ifdef USE_DBUS
    {"existing", 'e', 0, G_OPTION_ARG_NONE, &arg_existing,
     N_("Open URL or execute command in an existing HexChat"), NULL},
#endif
    {"minimize", 0, 0, G_OPTION_ARG_INT, &arg_minimize,
     N_("Begin minimized. Level 0=Normal 1=Iconified 2=Tray"), N_("level")},
    {"version", 'v', 0, G_OPTION_ARG_NONE, &arg_show_version,
     N_("Show version information"), NULL},
    {G_OPTION_REMAINING, '\0', 0, G_OPTION_ARG_STRING_ARRAY, &arg_urls,
     N_("Open an irc://server:port/channel?key URL"), "URL"},
    {NULL}};

#ifdef WIN32
static void create_msg_dialog(gchar *title, gchar *message) {
  GtkWidget *dialog;
...
/* On Win32 we automatically have the icon. If we try to load it explicitly, it
 * will look ugly for some reason. */
#ifndef WIN32
  pixmaps_init();
...
int fe_args(int argc, char *argv[]) {
  ...
  g_option_context_set_help_enabled(
      context, FALSE); /* disable stdout help as stdout is unavailable for
                          subsystem:windows */
#endif
...
#ifdef WIN32
  if (error) /* workaround for argv not being available when using
                subsystem:windows */
  {
    if (error->message) /* the error message contains argv so search for
                           patterns in that */
    {
      if (strstr(error->message, "--help-all") != NULL) {
        ...
        g_free(buffer);
        return 0;
      } else if (strstr(error->message, "--help") != NULL ||
  ...
  if (arg_show_autoload) {
    buffer = g_strdup_printf("%s%caddons%c", get_xdir(), G_DIR_SEPARATOR,
                             G_DIR_SEPARATOR);
#ifdef WIN32
    gtk_init(&argc, &argv);
...
GtkStyle *create_input_style(GtkStyle *style) {
  char buf[256];
...
#ifdef HAVE_GTK_MAC
static void gtkosx_application_terminate(GtkosxApplication *app,
                                         gpointer userdata) {
  hexchat_exit();
}
#endif
...
static void log_handler(const gchar *log_domain, GLogLevelFlags log_level,
  ...
  /* if (getenv ("HEXCHAT_WARNING_IGNORE")) this gets ignored sometimes, so
   * simply just disable all warnings */
  return;
...
static int fe_idle(gpointer data) {
  session *sess = sess_list->data;

  plugin_add(sess, NULL, NULL, notification_plugin_init,
             notification_plugin_deinit, NULL, FALSE);

  plugin_add(sess, NULL, NULL, tray_plugin_init, tray_plugin_deinit, NULL,
             FALSE);
...
void fe_new_window(session *sess, int focus) {
  int tab = FALSE;
...
void fe_set_topic(session *sess, char *topic, char *stripped_topic) {
...
static void fe_update_mode_entry(session *sess, GtkWidget *entry, char **text,
                                 char *new_text) {
  if (!sess->gui->is_tab || sess == current_tab) {
...
void fe_update_channel_key(struct session *sess) {
  fe_update_mode_entry(sess, sess->gui->key_entry, &sess->res->key_text,
                       sess->channelkey);
  fe_set_title(sess);
}
...
void fe_update_channel_limit(struct session *sess) {
  ...
  fe_update_mode_entry(sess, sess->gui->limit_entry, &sess->res->limit_text,
                       tmp);
  fe_set_title(sess);
}
...
void fe_progressbar_start(session *sess) {
...
void fe_print_text(struct session *sess, char *text, time_t stamp,
  ...
  PrintTextRaw(sess->res->buffer, (unsigned char *)text, prefs.hex_text_indent,
               stamp);
...
void fe_beep(session *sess) {
  ...
  if (!PlaySoundW(L"Notification.IM", NULL, SND_ALIAS | SND_ASYNC)) {
    /* The user does not have the "Instant Message Notification" sound set. Fall
     * back to system beep.
     */
    Beep(1000, 50);
  }
  ...
  if (ca_context_play(ca_con, 0, CA_PROP_EVENT_ID, "message-new-instant",
                      NULL) != 0)
#endif
    gdk_beep();
#endif
    ...
    if (sess->server == serv) {
      ...
      if (!sess->gui->is_tab || current_tab == sess) {
        if (sess->gui->lagometer) {
          gtk_progress_bar_set_fraction((GtkProgressBar *)sess->gui->lagometer,
                                        per);
          gtk_widget_set_tooltip_text(
              gtk_widget_get_parent(sess->gui->lagometer), lagtip);
        }
        if (sess->gui->laginfo)
...
void fe_set_throttle(server *serv) {
  ...
  while (list) {
    ...
    if (sess->server == serv) {
      ...
      g_snprintf(tip, sizeof(tip) - 1, _("Network send queue: %d bytes"),
                 serv->sendq_len);
      ...
      if (!sess->gui->is_tab || current_tab == sess) {
        if (sess->gui->throttlemeter) {
          gtk_progress_bar_set_fraction(
              (GtkProgressBar *)sess->gui->throttlemeter, per);
          gtk_widget_set_tooltip_text(
              gtk_widget_get_parent(sess->gui->throttlemeter), tip);
        }
...
static void dcc_saveas_cb(struct DCC *dcc, char *file) {
  if (is_dcc(dcc)) {
...
void *fe_gui_info_ptr(session *sess, int info_type) {
  ...
  case 0: /* native window pointer (for plugins) */
    ...
    return gdk_win32_window_get_impl_hwnd(
        gtk_widget_get_window(sess->gui->window));
#else
...
int fe_get_inputbox_cursor(session *sess) {
...
#ifdef __APPLE__
static char *url_escape_hostname(const char *url) {
...
static void osx_show_uri(const char *url) {
  ...
  if (encoded_url) {
    char *quoted_url = g_shell_quote(encoded_url);
    open = g_find_program_in_path("open");
    cmd = g_strjoin(" ", open, quoted_url, NULL);
...
static inline char *escape_uri(const char *uri) {
  return g_uri_escape_string(uri,
                             G_URI_RESERVED_CHARS_GENERIC_DELIMITERS
                                 G_URI_RESERVED_CHARS_SUBCOMPONENT_DELIMITERS,
                             FALSE);
}

static inline gboolean uri_contains_forbidden_characters(const char *uri) {
  while (*uri) {
...
static void fe_open_url_inner(const char *url) {
#ifdef WIN32
...

{
  ...
  gtkutil_file_req(NULL, title, callback, userdata, initial, NULL,
                   flags | FRF_FILTERISINITIAL);
}
