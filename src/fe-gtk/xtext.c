...
enum { WORD_CLICK, SET_SCROLL_ADJUSTMENTS, LAST_SIGNAL };
...
/* static char *gtk_xtext_conv_color (unsigned char *text, int len, int
 * *newlen); */
...
static unsigned char *gtk_xtext_strip_color(unsigned char *text, int len,
                                            unsigned char *outbuf, int *newlen,
                                            GSList **slp, int strip_hidden);
static gboolean gtk_xtext_check_ent_visibility(GtkXText *xtext,
                                               textentry *find_ent, int add);
static int gtk_xtext_render_page_timeout(GtkXText *xtext);
static int gtk_xtext_search_offset(xtext_buffer *buf, textentry *ent,
                                   unsigned int off);
static GList *gtk_xtext_search_textentry(xtext_buffer *, textentry *);
static void gtk_xtext_search_textentry_add(xtext_buffer *, textentry *, GList *,
                                           gboolean);
...
static gboolean gtk_xtext_search_init(xtext_buffer *buf, const gchar *text,
                                      gtk_xtext_search_flags flags,
                                      GError **perr);
static char *gtk_xtext_get_word(GtkXText *xtext, int x, int y,
                                textentry **ret_ent, int *ret_off, int *ret_len,
                                GSList **slp);

#define xtext_draw_bg(xt, x, y, w, h)                                          \
...
static PangoAttribute *xtext_pango_attr(PangoAttribute *attr) {
  attr->start_index = PANGO_ATTR_INDEX_FROM_TEXT_BEGINNING;
  attr->end_index = PANGO_ATTR_INDEX_TO_TEXT_END;
  return attr;
}

static void xtext_pango_init(GtkXText *xtext) {
  size_t i;
  int j;
  char buf[2] = "\000";
...
static int backend_get_text_width_emph(GtkXText *xtext, guchar *str, int len,
                                       int emphasis) {
  int width;
  int deltaw;
  int mbl;
...
static int backend_get_text_width_slp(GtkXText *xtext, guchar *str,
                                      GSList *slp) {
  int width = 0;
...
static void xtext_draw_layout_line(GdkDrawable *drawable, GdkGC *gc, gint x,
  ...
  while (tmp_list) {
    ...
    pango_glyph_string_extents(run->glyphs, run->item->analysis.font, NULL,
                               &logical_rect);
...
static void backend_draw_text_emph(GtkXText *xtext, int dofill, GdkGC *gc,
                                   int x, int y, char *str, int len,
                                   int str_width, int emphasis) {
  GdkGCValues val;
  GdkColor col;
...
static void gtk_xtext_init(GtkXText *xtext) {
...

  {
    ...
    gtk_selection_add_targets(GTK_WIDGET(xtext), GDK_SELECTION_PRIMARY, targets,
                              n_targets);
  }
}
...
static void gtk_xtext_adjustment_set(xtext_buffer *buf, int fire_signal) {
  ...
  if (buf->xtext->buffer == buf) {
    ...
    adj->page_size =
        GTK_WIDGET(buf->xtext)->allocation.height / buf->xtext->fontsize;
...
static void gtk_xtext_adjustment_changed(GtkAdjustment *adj, GtkXText *xtext) {
  ...
  if (xtext->buffer->old_value != xtext->adj->value) {
    ...
    if (xtext->adj->value + 1 == xtext->buffer->old_value ||
        xtext->adj->value - 1 ==
            xtext->buffer->old_value) /* clicked an arrow? */
    {
      if (xtext->io_tag) {
      ...
      if (!xtext->io_tag)
        xtext->io_tag = g_timeout_add(
            REFRESH_TIMEOUT, (GSourceFunc)gtk_xtext_adjustment_timeout, xtext);
    }
  }
...
GtkWidget *gtk_xtext_new(GdkColor palette[], int separator) {
  ...
  gtk_widget_set_double_buffered(GTK_WIDGET(xtext), TRUE);
...
static void gtk_xtext_destroy(GtkObject *object) {
  ...
  if (xtext->adj) {
    g_signal_handlers_disconnect_matched(
        G_OBJECT(xtext->adj), G_SIGNAL_MATCH_DATA, 0, 0, NULL, NULL, xtext);
...
static void gtk_xtext_realize(GtkWidget *widget) {
  ...
  xtext->thin_gc = gdk_gc_new_with_values(widget->window, &val,
                                          GDK_GC_EXPOSURES | GDK_GC_SUBWINDOW);
  xtext->marker_gc = gdk_gc_new_with_values(
      widget->window, &val, GDK_GC_EXPOSURES | GDK_GC_SUBWINDOW);
  ...
  xtext->hand_cursor = gdk_cursor_new_for_display(
      gdk_window_get_display(widget->window), GDK_HAND1);
  xtext->resize_cursor = gdk_cursor_new_for_display(
      gdk_window_get_display(widget->window), GDK_LEFT_SIDE);
...
static int find_x(GtkXText *xtext, textentry *ent, int x, int subline,
                  int indent) {
  int xx = indent;
  int suboff;
  GSList *list;
...
static int gtk_xtext_find_x(GtkXText *xtext, int x, textentry *ent, int subline,
...
static textentry *gtk_xtext_find_char(GtkXText *xtext, int x, int y, int *off,
                                      int *out_of_bounds) {
  textentry *ent;
  int line;
  int subline;
...
static void gtk_xtext_draw_marker(GtkXText *xtext, textentry *ent, int y) {
  ...
  if (xtext->buffer->marker_pos == ent) {
    ...
    render_y = y + xtext->font->descent +
               xtext->fontsize * g_slist_length(ent->sublines);
  } else
    return;
  ...
  gdk_draw_line(xtext->draw_buf, xtext->marker_gc, x, render_y, x + width,
                render_y);

  if (gtk_window_has_toplevel_focus(
          GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(xtext))))) {
...
static void gtk_xtext_selection_up(GtkXText *xtext, textentry *start,
                                   textentry *end, int start_offset) {
...
static void gtk_xtext_selection_down(GtkXText *xtext, textentry *start,
                                     textentry *end, int end_offset) {
  /* render all the complete lines */
  if (end->prev == start)
    gtk_xtext_render_ents(xtext, start, NULL);
  else
...
static void gtk_xtext_selection_render(GtkXText *xtext, textentry *start_ent,
                                       textentry *end_ent) {
  textentry *ent;
...
static void gtk_xtext_selection_draw(GtkXText *xtext, GdkEventMotion *event,
                                     gboolean render) {
  textentry *ent;
  textentry *ent_end;
  textentry *ent_start;
  ...
  ent_start = gtk_xtext_find_char(xtext, xtext->select_start_x,
                                  xtext->select_start_y, &offset_start, NULL);
  ent_end = gtk_xtext_find_char(xtext, xtext->select_end_x, xtext->select_end_y,
                                &offset_end, NULL);
  if (ent_start == NULL && ent_end == NULL)
    return;

  if ((ent_start != ent_end &&
       xtext->select_start_y > xtext->select_end_y) || /* different entries */
      (ent_start == ent_end &&
       offset_start > offset_end)) /* same entry, different character offsets */
  {
    /* marking up */
    low_ent = ent_end;
  ...
  if (xtext->word_select) {
    /* a word selection cannot be started if the cursor is out of bounds in
     * gtk_xtext_button_press */
    gtk_xtext_get_word(xtext, low_x, low_y, NULL, &low_offs, NULL, NULL);

    /* in case the cursor is out of bounds we keep offset_end from
     * gtk_xtext_find_char and fix the length */
    if (gtk_xtext_get_word(xtext, high_x, high_y, NULL, &high_offs, &high_len,
                           NULL) == NULL)
      high_len = high_offs == high_ent->str_len
                     ? 0
                     : -1; /* -1 for the space, 0 if at the end */
    high_offs += high_len;
    if (low_y < 0)
...
static gint gtk_xtext_scrolldown_timeout(GtkXText *xtext) {
  ...
  xtext->scroll_tag =
      g_timeout_add(gtk_xtext_timeout_ms(xtext, p_y - win_height),
                    (GSourceFunc)gtk_xtext_scrolldown_timeout, xtext);
...
static gint gtk_xtext_scrollup_timeout(GtkXText *xtext) {
  ...
  xtext->scroll_tag =
      g_timeout_add(gtk_xtext_timeout_ms(xtext, p_y),
                    (GSourceFunc)gtk_xtext_scrollup_timeout, xtext);

  return 0;
}

static void gtk_xtext_selection_update(GtkXText *xtext, GdkEventMotion *event,
                                       int p_y, gboolean render) {
  int win_height;
  int moved;
...
static char *gtk_xtext_get_word(GtkXText *xtext, int x, int y,
  ...
  gtk_xtext_strip_color(word, len_to_offset, xtext->scratch_buffer,
                        &len_to_offset, NULL, FALSE);
  ...
  word =
      gtk_xtext_strip_color(word, len, xtext->scratch_buffer, NULL, slp, FALSE);

  /* avoid turning the cursor into a hand for non-url part of the word */
  if (xtext->urlcheck_function &&
      xtext->urlcheck_function(GTK_WIDGET(xtext), word)) {
    int start, end;
    url_last(&start, &end);
...
static int gtk_xtext_get_word_adjust(GtkXText *xtext, int x, int y,
                                     textentry **word_ent, int *offset,
                                     int *len) {
  GSList *slp = NULL;
  unsigned char *word;
  int word_type = 0;
...
static gboolean gtk_xtext_motion_notify(GtkWidget *widget,
  ...
  if (xtext->separator && xtext->buffer->indent) {
    ...
    if (line_x == x || line_x == x + 1 || line_x == x - 1) {
      if (!xtext->cursor_resize) {
        gdk_window_set_cursor(GTK_WIDGET(xtext)->window, xtext->resize_cursor);
        xtext->cursor_hand = FALSE;
  ...
  if (word_type > 0) {
    if (!xtext->cursor_hand || xtext->hilight_ent != word_ent ||
  ...
  if (str) {
    if (str[0]) {
      gtk_clipboard_set_text(
          gtk_widget_get_clipboard(xtext, GDK_SELECTION_CLIPBOARD), str, len);

      gtk_selection_owner_set(xtext, GDK_SELECTION_PRIMARY,
                              event ? event->time : GDK_CURRENT_TIME);
      gtk_selection_owner_set(xtext, GDK_SELECTION_SECONDARY,
                              event ? event->time : GDK_CURRENT_TIME);
    }
...
static void gtk_xtext_selection_get(GtkWidget *widget,
  ...
  default:
    ...
    gtk_selection_data_set(selection_data_ptr, GDK_SELECTION_TYPE_STRING, 8,
                           new_text, glen);
    g_free(new_text);
  }
...
static void gtk_xtext_scroll_adjustments(GtkXText *xtext, GtkAdjustment *hadj,
  ...
  if (xtext->adj && (xtext->adj != vadj)) {
    g_signal_handlers_disconnect_by_func(xtext->adj,
                                         gtk_xtext_adjustment_changed, xtext);
  ...
  if (xtext->adj != vadj) {
    ...
    xtext->vc_signal_tag =
        g_signal_connect(xtext->adj, "value-changed",
...
static void gtk_xtext_class_init(GtkXTextClass *class) {
  ...
  xtext_signals[WORD_CLICK] =
      g_signal_new("word_click", G_TYPE_FROM_CLASS(object_class),
                   G_SIGNAL_RUN_FIRST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(GtkXTextClass, word_click), NULL, NULL,
                   _hexchat_marshal_VOID__POINTER_POINTER, G_TYPE_NONE, 2,
                   G_TYPE_POINTER, G_TYPE_POINTER);
  xtext_signals[SET_SCROLL_ADJUSTMENTS] =
      g_signal_new("set_scroll_adjustments", G_OBJECT_CLASS_TYPE(object_class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(GtkXTextClass, set_scroll_adjustments), NULL,
                   NULL, _hexchat_marshal_VOID__OBJECT_OBJECT, G_TYPE_NONE, 2,
...
GType gtk_xtext_get_type(void) {
  ...
  if (!xtext_type) {
    ...
    xtext_type =
        g_type_register_static(GTK_TYPE_WIDGET, "GtkXText", &xtext_info, 0);
  }
...
static unsigned char *gtk_xtext_strip_color(unsigned char *text, int len,
  ...
  while (len > 0) {
    ...
    if (rcol > 0 &&
        (isdigit(*text) || (*text == ',' && isdigit(text[1]) && !bgcol))) {
      if (text[1] != ',')
        rcol--;
      if (*text == ',') {
        rcol = 2;
...
static int gtk_xtext_text_width_ent(GtkXText *xtext, textentry *ent) {
  ...
  for (slp = slp0; slp; slp = g_slist_next(slp)) {
    ...
    meta = slp->data;
    meta->width = backend_get_text_width_emph(xtext, ent->str + meta->off,
                                              meta->len, meta->emph);
  }
  return width;
}
...
static int gtk_xtext_render_flush(GtkXText *xtext, int x, int y,
                                  unsigned char *str, int len, GdkGC *gc,
                                  int *emphasis) {
  ...
  pix =
      gdk_pixmap_new(xtext->draw_buf, str_width, xtext->fontsize, xtext->depth);
  if (pix) {
    dest_x = x;
  ...
  backend_draw_text_emph(xtext, dofill, gc, x, y, str, len, str_width,
                         *emphasis);
  ...
  if (pix) {
    ...
    if (gdk_rectangle_intersect(&clip, &dest, &dest))
      ...
      gdk_draw_drawable(xtext->draw_buf, xtext->bgc, pix, dest.x - dest_x,
                        dest.y - dest_y, dest.x, dest.y, dest.width,
                        dest.height);
    g_object_unref(pix);
  }

  if (xtext->strikethrough) {
    /* pango_attr_strikethrough_new does not render in the custom widget so we
...
static int gtk_xtext_render_str(GtkXText *xtext, int y, textentry *ent,
                                unsigned char *str, int len, int win_width,
                                int indent, int line, int left_only,
                                int *x_size_ret, int *emphasis) {
  GdkGC *gc;
  ...
  if (ent->mark_start != -1 && ent->mark_start <= i + offset &&
  ...
  while (i < len) {
    ...
    if ((xtext->parsing_color && isdigit(str[i]) && xtext->nc < 2) ||
        (xtext->parsing_color && str[i] == ',' && isdigit(str[i + 1]) &&
         xtext->nc < 3 && !xtext->parsing_backcolor)) {
      pstr++;
      if (str[i] == ',') {
...
static int find_next_wrap(GtkXText *xtext, textentry *ent, unsigned char *str,
  ...
  while (1) {
    if (rcol > 0 &&
        (isdigit(*str) || (*str == ',' && isdigit(str[1]) && !bgcol))) {
      if (str[1] != ',')
        rcol--;
      if (*str == ',') {
        rcol = 2;
...
static void gtk_xtext_render_stamp(GtkXText *xtext, textentry *ent, char *text,
                                   int len, int line, int win_width) {
  textentry tmp_ent;
  int jo, ji, hs;
  ...
  gtk_xtext_render_str(xtext, y, ent, text, len, win_width, 2, line, TRUE,
                       &xsize, &emphasis);
...
static int gtk_xtext_render_line(GtkXText *xtext, textentry *ent, int line,
  ...
  do {
    if (entline > 0)
      len = GPOINTER_TO_INT(g_slist_nth_data(ent->sublines, entline)) -
            GPOINTER_TO_INT(g_slist_nth_data(ent->sublines, entline - 1));
    else
      len = GPOINTER_TO_INT(g_slist_nth_data(ent->sublines, entline));
    ...
    y = (xtext->fontsize * line) + xtext->font->ascent - xtext->pixel_offset;
    if (!subline) {
      if (!gtk_xtext_render_str(xtext, y, ent, str, len, win_width, indent,
                                line, FALSE, NULL, &emphasis)) {
        /* small optimization */
        gtk_xtext_draw_marker(
            xtext, ent, y - xtext->fontsize * (taken + start_subline + 1));
        return g_slist_length(ent->sublines) - subline;
      }
    } else {
      xtext->dont_render = TRUE;
      gtk_xtext_render_str(xtext, y, ent, str, len, win_width, indent, line,
                           FALSE, NULL, &emphasis);
      xtext->dont_render = FALSE;
      subline--;
      line--;
  ...
  gtk_xtext_draw_marker(xtext, ent,
                        y - xtext->fontsize * (taken + start_subline));
...
static void gtk_xtext_recalc_widths(xtext_buffer *buf, int do_str_width) {
  ...
  while (ent) {
    ...
    if (ent->left_len != -1) {
      ent->indent = (buf->indent - gtk_xtext_text_width(buf->xtext, ent->str,
                                                        ent->left_len)) -
...
void gtk_xtext_set_background(GtkXText *xtext, GdkPixmap *pixmap) {
  ...
  if (pixmap != 0) {
    ...
    val.subwindow_mode = GDK_INCLUDE_INFERIORS;
    val.graphics_exposures = 0;
    xtext->bgc = gdk_gc_new_with_values(GTK_WIDGET(xtext)->window, &val,
                                        GDK_GC_EXPOSURES | GDK_GC_SUBWINDOW);
    xtext_set_fg(xtext, xtext->bgc, XTEXT_BG);
  }
}
...
void gtk_xtext_save(GtkXText *xtext, int fh) {
  ...
  while (ent) {
    buf = gtk_xtext_strip_color(ent->str, ent->str_len, NULL, &newlen, NULL,
                                FALSE);
    write(fh, buf, newlen);
    write(fh, "\n", 1);
    g_free(buf);
...
static int gtk_xtext_lines_taken(xtext_buffer *buf, textentry *ent) {
  ...
  if (win_width >= ent->indent + ent->str_width) {
    ent->sublines =
        g_slist_append(ent->sublines, GINT_TO_POINTER(ent->str_len));
    return 1;
  }
  ...
  do {
    ...
    ent->sublines =
        g_slist_append(ent->sublines, GINT_TO_POINTER(str + len - ent->str));
    indent = buf->indent;
    str += len;
  } while (str < ent->str + ent->str_len);
...
static int gtk_xtext_render_ents(GtkXText *xtext, textentry *enta,
                                 textentry *entb) {
  ...
  if (width < 32 || height < xtext->fontsize ||
      width < xtext->buffer->indent + 30)
    return 0;
  ...
  while (ent) {
    ...
    if (drawing || ent == entb || ent == enta) {
      ...
      line +=
          gtk_xtext_render_line(xtext, ent, line, lines_max, subline, width);
      subline = 0;
...
static void gtk_xtext_render_page(GtkXText *xtext) {
  ...
  if (width < 34 || height < xtext->fontsize ||
      width < xtext->buffer->indent + 32)
    return;
...
#ifndef __APPLE__
  /* Scrolling optimization (copying previous window content) is prone to
     artifacts (repetitive icons) on modern compositors or if the background
     isn't cleared perfectly. Disabling it forces a full redraw, which is safer
     and fast enough. */
  if (0 && !xtext->pixmap && abs(overlap) < height) {
    GdkRectangle area;
    ...
    if (overlap < 1) /* DOWN */
    {
      ...
      gdk_draw_drawable(xtext->draw_buf, xtext->fgc, xtext->draw_buf, 0,
                        -overlap, 0, 0, width, height + overlap);
      remainder = ((height - xtext->font->descent) % xtext->fontsize) +
                  xtext->font->descent;
      area.y = (height + overlap) - remainder;
      area.height = remainder - overlap;
    } else {
      gdk_draw_drawable(xtext->draw_buf, xtext->fgc, xtext->draw_buf, 0, 0, 0,
                        overlap, width, height - overlap);
      area.y = 0;
      area.height = overlap;
    }
  ...
  /* Clear the entire background first to ensure the theme color is applied
     everywhere, especially when switching themes. */
  xtext_draw_bg(xtext, 0, 0, width + MARGIN, height);
...
static int gtk_xtext_kill_ent(xtext_buffer *buffer, textentry *ent) {
  ...
  if (buffer->marker_pos == ent) {
    /* Allow for "Marker line reset because exceeded scrollback limit. to
     * appear. */
...
static void gtk_xtext_remove_top(xtext_buffer *buffer) {
  ...
  buffer->last_pixel_pos -=
      (g_slist_length(ent->sublines) * buffer->xtext->fontsize);
  ...
  if (gtk_xtext_kill_ent(buffer, ent)) {
    if (!buffer->xtext->add_io_tag) {
      ...
      buffer->xtext->add_io_tag = g_timeout_add(
          REFRESH_TIMEOUT * 2, (GSourceFunc)gtk_xtext_render_page_timeout,
          buffer->xtext);
    }
  }
}

static void gtk_xtext_remove_bottom(xtext_buffer *buffer) {
  textentry *ent;
...
static gboolean gtk_xtext_check_ent_visibility(GtkXText *xtext,
  ...
  lines = ((height + xtext->pixel_offset) / xtext->fontsize) +
          buf->pagetop_subline + add;
  while (ent) {
    lines -= g_slist_length(ent->sublines);
    if (lines <= 0) {
      return FALSE;
...
static void gtk_xtext_unstrip_color(gint start, gint end, GSList *slp,
                                    GList **gl, gint maxo) {
...
static GList *gtk_xtext_search_textentry(xtext_buffer *buf, textentry *ent) {
  ...
  str =
      gtk_xtext_strip_color(ent->str, ent->str_len, buf->xtext->scratch_buffer,
  ...
  if (buf->search_flags & regexp) {
    ...
    for (pos = hay, len = lhay; len;
      ...
      off = str - hay;
      gtk_xtext_unstrip_color(off, off + buf->search_lnee, slp, &gl,
                              ent->str_len);
    }
...
static void gtk_xtext_search_textentry_add(xtext_buffer *buf, textentry *ent,
                                           GList *gl, gboolean pre) {
  ent->marks = gl;
  if (gl) {
    buf->search_found =
        (pre ? g_list_prepend : g_list_append)(buf->search_found, ent);
...
static void gtk_xtext_search_textentry_del(xtext_buffer *buf, textentry *ent) {
...
static void gtk_xtext_search_textentry_fini(gpointer entp, gpointer dummy) {
  textentry *ent = entp;
...
static gboolean gtk_xtext_search_init(xtext_buffer *buf, const gchar *text,
  ...
  if (buf->search_found && strcmp(buf->search_text, text) == 0 &&
  ...
  if (flags & regexp) {
    buf->search_re =
        g_regex_new(text, (flags & case_match) ? 0 : G_REGEX_CASELESS, 0, perr);
    if (perr && *perr) {
      return FALSE;
    }
  } else {
...
textentry *gtk_xtext_search(GtkXText *xtext, const gchar *text,
                            gtk_xtext_search_flags flags, GError **perr) {
  textentry *ent = NULL;
  xtext_buffer *buf = xtext->buffer;
  GList *gl;
  ...
  /* if the text arg is "", the reset button has been clicked or Control-Shift-F
     has been hit */
  ...
  /* If the text arg is neither NULL nor "", it's the search string */
    ...
    if (gtk_xtext_search_init(buf, text, flags, perr) ==
        FALSE) /* If a new search: */
    {
      if (perr && *perr) {
        return NULL;
      }
    ...
    if (buf->search_found) {
      ...
      buf->curdata.u =
          (buf->curmark) ? GPOINTER_TO_UINT(buf->curmark->data) : 0;
    }
  }
  buf->hintsearch = ent;
  ...
  if (!gtk_xtext_check_ent_visibility(xtext, ent, 1)) {
    ...
    buf->pagetop_ent = NULL;
    for (value = 0, ent = buf->text_first; ent && ent != buf->hintsearch;
         ent = ent->next) {
...
static void gtk_xtext_append_entry(xtext_buffer *buf, textentry *ent,
  ...
  if ((buf->marker_pos == NULL || buf->marker_seen) &&
      (buf->xtext->buffer != buf ||
       !gtk_window_has_toplevel_focus(
           GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(buf->xtext)))))) {
    buf->marker_pos = ent;
  ...
  if (buf->xtext->buffer == buf) {
    ...
    if (!buf->xtext->add_io_tag) {
      ...
      buf->xtext->add_io_tag =
          g_timeout_add(REFRESH_TIMEOUT * 2,
                        (GSourceFunc)gtk_xtext_render_page_timeout, buf->xtext);
    }
  }
...
void gtk_xtext_append_indent(xtext_buffer *buf, unsigned char *left_text,
                             int left_len, unsigned char *right_text,
  ...
  if (buf->xtext->auto_indent && buf->indent < buf->xtext->max_auto_indent &&
