...
void set_topic(session *sess, char *topic, char *stripped_topic) {
  ...
  if (sess->type == SESS_DIALOG &&
      (!sess->topic || strcmp(sess->topic, stripped_topic)) &&
      sess->logfd != -1) {
    char tbuf[1024];
...
static session *find_session_from_nick(char *nick, server *serv) {
  session *sess;
  ...
  if (serv->front_session) {
    ...
    if (!g_ascii_strcasecmp(nick, "ChanServ") ||
        userlist_find(serv->front_session, nick))
...
static session *inbound_open_dialog(server *serv, char *from,
...
static void inbound_make_idtext(server *serv, char *idtext, int max, int id) {
  idtext[0] = 0;
...
void inbound_privmsg(server *serv, char *from, char *ip, char *text, int id,
  ...
  if (sess || prefs.hex_gui_autoopen_dialog) {
    ...
    inbound_chanmsg(serv, NULL, NULL, from, text, FALSE, tags_data->identified,
                    tags_data);
    return;
  }
  ...
  if (!sess) {
    ...
    nodiag =
        TRUE; /* We don't want it to look like a normal message in front sess */
  }
...
static int is_hilight(char *from, char *text, session *sess, server *serv) {
...
void inbound_action(session *sess, char *chan, char *from, char *ip, char *text,
  ...
  if (fromme)
    EMIT_SIGNAL_TIMESTAMP(XP_TE_UACTION, sess, from, text, nickchar, idtext, 0,
                          tags_data->timestamp);
  else if (!privaction)
    EMIT_SIGNAL_TIMESTAMP(XP_TE_CHANACTION, sess, from, text, nickchar, idtext,
                          0, tags_data->timestamp);
  else if (sess->type == SESS_DIALOG)
    EMIT_SIGNAL_TIMESTAMP(XP_TE_DPRIVACTION, sess, from, text, idtext, NULL, 0,
                          tags_data->timestamp);
  else
...
void inbound_chanmsg(server *serv, session *sess, char *chan, char *from,
  ...
  if (fromme) {
    ...
    EMIT_SIGNAL_TIMESTAMP(XP_TE_UCHANMSG, sess, from, text, nickchar, NULL, 0,
                          tags_data->timestamp);
    return;
  }
  ...
  if (sess->type == SESS_DIALOG)
    ...
    EMIT_SIGNAL_TIMESTAMP(XP_TE_HCHANMSG, sess, from, text, nickchar, idtext, 0,
                          tags_data->timestamp);
  else
    EMIT_SIGNAL_TIMESTAMP(XP_TE_CHANMSG, sess, from, text, nickchar, idtext, 0,
                          tags_data->timestamp);
}
...
void inbound_newnick(server *serv, char *nick, char *newnick, int quiet,
  ...
  while (list) {
    sess = list->data;
    if (sess->server == serv) {
      if (userlist_change(sess, nick, newnick) ||
          (me && sess->type == SESS_SERVER)) {
        if (!quiet) {
          if (me)
            EMIT_SIGNAL_TIMESTAMP(XP_TE_UCHANGENICK, sess, nick, newnick, NULL,
                                  NULL, 0, tags_data->timestamp);
          else
            EMIT_SIGNAL_TIMESTAMP(XP_TE_CHANGENICK, sess, nick, newnick, NULL,
                                  NULL, 0, tags_data->timestamp);
        }
      }
...
/* find a "<none>" tab */
static session *find_unused_session(server *serv) {
  session *sess;
  GSList *list = sess_list;
  while (list) {
...
static session *find_session_from_waitchannel(char *chan, struct server *serv) {
  session *sess;
  GSList *list = sess_list;
  while (list) {
    sess = (session *)list->data;
...
void inbound_ukick(server *serv, char *chan, char *kicker, char *reason,
  ...
  if (sess) {
    EMIT_SIGNAL_TIMESTAMP(XP_TE_UKICK, sess, serv->nick, chan, kicker, reason,
                          0, tags_data->timestamp);
    clear_channel(sess);
...
void inbound_upart(server *serv, char *chan, char *ip, char *reason,
  ...
  if (sess) {
    if (*reason)
      ...
      EMIT_SIGNAL_TIMESTAMP(XP_TE_UPART, sess, serv->nick, ip, chan, NULL, 0,
                            tags_data->timestamp);
    clear_channel(sess);
  }
}
...
void inbound_nameslist(server *serv, char *chan, char *names,
  ...
  sess = find_channel(serv, chan);
  if (!sess) {
    EMIT_SIGNAL_TIMESTAMP(XP_TE_USERSONCHAN, serv->server_session, chan, names,
                          NULL, NULL, 0, tags_data->timestamp);
    return;
  }
  if (!sess->ignore_names)
    EMIT_SIGNAL_TIMESTAMP(XP_TE_USERSONCHAN, sess, chan, names, NULL, NULL, 0,
                          tags_data->timestamp);
...
void inbound_join(server *serv, char *chan, char *user, char *ip, char *account,
...
void inbound_part(server *serv, char *chan, char *user, char *ip, char *reason,
  ...
  if (sess) {
    if (*reason)
      EMIT_SIGNAL_TIMESTAMP(XP_TE_PARTREASON, sess, user, ip, chan, reason, 0,
                            tags_data->timestamp);
    else
...
static session *find_session_from_type(int type, server *serv) {
  session *sess;
  GSList *list = sess_list;
  while (list) {
    sess = list->data;
    ...
    if (sess->server == serv) {
      ...
      if (sess == serv->front_session && notify_is_in_list(serv, nick)) {
        if (reason)
          ...
          EMIT_SIGNAL_TIMESTAMP(XP_TE_NOTIFYBACK, sess, nick, NULL, NULL, NULL,
                                0, tags_data->timestamp);
      }
    }
    list = list->next;
...
static gboolean check_autojoin_channels(server *serv) {
  int i = 0;
  session *sess;
  GSList *list = sess_list;
  GSList *sess_channels =
      NULL; /* joined channels that are not in the favorites list */
  favchannel *fav;
  ...
  /* If there's a session (i.e. this is a reconnect), autojoin to everything
   * that was open previously. */
  while (list) {
    sess = list->data;
    ...
    if (sess->server == serv) {
      if (sess->willjoinchannel[0] != 0) {
        ...
        fav =
            servlist_favchan_find(serv->network, sess->waitchannel,
                                  NULL); /* Is this channel in our favorites? */

        /* session->channelkey is initially unset for channels joined from the
         * favorites. You have to fill them up manually from favorites settings.
         */
        if (fav) {
          /* session->channelkey is set if there was a key change during the
           * session. In that case, use the session key, not the one from
           * favorites. */
        ...
        /* for easier checks, ensure that favchannel->key is just NULL when
         * session->channelkey is empty i.e. '' */
        if (strlen(sess->channelkey)) {
          sess_channels = servlist_favchan_listadd(
              sess_channels, sess->waitchannel, sess->channelkey);
        } else {
          sess_channels =
              servlist_favchan_listadd(sess_channels, sess->waitchannel, NULL);
        }
        i++;
      }
    }
  ...
  if (sess_channels) {
    ...
    if (serv->favlist) {
      ...
      /* FIXME this is not going to work and is not needed either. server_free()
       * does the job already. */
      /* g_slist_free_full (serv->favlist, (GDestroyNotify)
       * servlist_favchan_free); */
    }
  }
...
void inbound_next_nick(session *sess, char *nick, int error,
  ...
  case 2:
    ...
    if (error) {
      EMIT_SIGNAL_TIMESTAMP(XP_TE_NICKERROR, sess, nick, newnick, NULL, NULL, 0,
                            tags_data->timestamp);
    } else {
      EMIT_SIGNAL_TIMESTAMP(XP_TE_NICKCLASH, sess, nick, newnick, NULL, NULL, 0,
                            tags_data->timestamp);
    }
    break;
...
static void dns_addr_callback(GObject *obj, GAsyncResult *result,
                              gpointer user_data) {
...
static void dns_name_callback(GObject *obj, GAsyncResult *result,
                              gpointer user_data) {
  GResolver *resolver = G_RESOLVER(obj);
  session *sess = (session *)user_data;
  GList *addrs;
...
void do_dns(session *sess, char *nick, char *host,
  ...
  if (addr)
    g_resolver_lookup_by_address_async(res, addr, NULL, dns_addr_callback,
                                       sess);
  else
    g_resolver_lookup_by_name_async(res, host, NULL, dns_name_callback, sess);
}

static void set_default_modes(server *serv) {
  char modes[8];
...
static void inbound_set_all_away_status(server *serv, char *nick,
                                        unsigned int status) {
  GSList *list;
  session *sess;
...
void inbound_user_info(session *sess, char *chan, char *user, char *host,
  ...
  if (chan) {
    ...
    if (who_sess)
      userlist_add_hostname(who_sess, nick, uhost, realname, servname, account,
                            away);
    else {
    ...
    for (list = sess_list; list; list = list->next) {
      ...
      if (sess->type == SESS_CHANNEL) {
        userlist_add_hostname(sess, nick, uhost, realname, servname, account,
                              away);
      } else if (sess->type == SESS_DIALOG && uhost &&
                 !serv->p_cmp(sess->channel, nick)) {
...
int inbound_banlist(session *sess, time_t stamp, char *chan, char *mask,
  ...
  nowindow:
    ...
    EMIT_SIGNAL_TIMESTAMP(XP_TE_BANLIST, sess, chan, mask, banner, time_str, 0,
                          tags_data->timestamp);
    return TRUE;
  }
...
static int inbound_exec_eom_cmd(char *str, void *sess) {
  char *cmd;
...
static int inbound_nickserv_login(server *serv) {
...
void inbound_login_end(session *sess, char *text,
  ...
  if (!serv->end_of_motd) {
    ...
    if (net && prefs.hex_irc_join_delay &&
        ((net->pass && inbound_nickserv_login(serv)) || net->commandlist)) {
      serv->joindelay_tag = fe_timeout_add_seconds(
          prefs.hex_irc_join_delay, check_autojoin_channels, serv);
    } else {
...
void inbound_identified(server *serv) /* 'MODE +e MYSELF' on freenode */
{
...
static const char *sasl_mechanisms[] = {"PLAIN", "EXTERNAL", "SCRAM-SHA-1",
                                        "SCRAM-SHA-256", "SCRAM-SHA-512"};

static void inbound_toggle_caps(server *serv, const char *extensions_str,
                                gboolean enable) {
  char **extensions;
  gsize i;
  ...
  for (i = 0; extensions[i]; i++) {
    ...
    else if (!strcmp(extension, "msgid"))
      serv->have_msgid = enable;
    ...
    else if (!strcmp(extension, "chghost"))
      serv->have_chghost = enable;
    else if (!strcmp(extension, "setname"))
      serv->have_setname = enable;
...
void inbound_cap_ls(server *serv, char *nick, char *extensions_str,
                    const message_tags_data *tags_data) {
  char buffer[500]; /* buffer for requesting capabilities and emitting the
                       signal */
  gboolean want_cap = FALSE; /* format the CAP REQ string based on previous
                                capabilities being requested or not */
  char **extensions;
  int i;
  ...
  for (i = 0; extensions[i]; i++) {
    ...
    /* if the SASL password is set AND auth mode is set to SASL, request SASL
     * auth */
  ...
  if (want_cap) {
    ...
    EMIT_SIGNAL_TIMESTAMP(XP_TE_CAPREQ, serv->server_session, buffer + 9, NULL,
                          NULL, NULL, 0, tags_data->timestamp);
    tcp_sendf(serv, "%s\r\n", g_strchomp(buffer));
  }
...
void inbound_cap_nak(server *serv, char *extensions_str,
                     const message_tags_data *tags_data) {
  char **extensions;
  int i;
 ...
 */
static void scram_authenticate(server *serv, const char *data,
                               const char *digest, const char *user,
                               const char *password) {
  ...
  if (serv->scram_session == NULL) {
    ...
    if (serv->scram_session == NULL) {
      PrintTextf(serv->server_session,
                 _("Could not create SCRAM session with digest %s"), digest);
  ...
  if (status == SCRAM_IN_PROGRESS) {
    ...
    if (serv->scram_session->error != NULL) {
      PrintTextf(serv->server_session,
                 _("SASL SCRAM authentication failed: %s"),
                 serv->scram_session->error);
      g_info("SASL SCRAM authentication failed: %s",
             serv->scram_session->error);
    }
...
void inbound_sasl_authenticate(server *serv, char *data) {
  ...
  EMIT_SIGNAL_TIMESTAMP(XP_TE_SASLAUTH, serv->server_session, user,
                        (char *)mech, NULL, NULL, 0, 0);
}
