...
const gchar *unicode_fallback_string =
    "\357\277\275"; /* The Unicode replacement character 0xFFFD */
...
static char *scrollback_get_filename(session *sess) {
  ...
  buf = g_strdup_printf("%s" G_DIR_SEPARATOR_S "scrollback" G_DIR_SEPARATOR_S
                        "%s" G_DIR_SEPARATOR_S "%s.txt",
                        get_xdir(), net, "");
  mkdir_p(buf);
  g_free(buf);
...
void scrollback_close(session *sess) { g_clear_object(&sess->scrollfile); }
...
static void scrollback_save(session *sess, char *text, time_t stamp) {
  ...
  ostream = G_OUTPUT_STREAM(
      g_file_append_to(sess->scrollfile, G_FILE_CREATE_PRIVATE, NULL, NULL));
  if (!ostream)
    return;
  ...
  if ((sess->scrollwritten > prefs.hex_text_max_lines &&
       prefs.hex_text_max_lines > 0) ||
...
void scrollback_load(session *sess) {
  ...
  while (1) {
    ...
    if (!err && buf) {
      /*
       * Some scrollback lines have three blanks after the timestamp and a
       * newline Some have only one blank and a newline Some don't even have a
       * timestamp Some don't have any text at all
       */
      if (buf[0] == 'T' && buf[1] == ' ') {
        if (sizeof(time_t) == 4)
          stamp = strtoul(buf + 2, NULL, 10);
        else
          stamp = g_ascii_strtoull(buf + 2, NULL,
                                   10); /* in case time_t is 64 bits */
      ...
      if (g_error_matches(err, G_CONVERT_ERROR,
...
static char *log_escape_strcpy(char *dest, char *src, char *end) {
  while (*src) {
    *dest = *src;
    if (dest + 1 == end)
      break;
...
static void log_insert_vars(char *buf, int bufsize, char *fmt, char *c, char *n,
                            char *s) {
...
static char *log_create_pathname(char *servname, char *channame,
                                 char *netname) {
  char fname[384];
  char fnametime[384];
  time_t now;
  ...
  log_insert_vars(fname, sizeof(fname), prefs.hex_irc_logmask, channame,
                  netname, servname);
  g_free(channame);
  g_free(netname);
  g_free(servname);
  ...
  {
    g_snprintf(fname, sizeof(fname),
               "%s" G_DIR_SEPARATOR_S "logs" G_DIR_SEPARATOR_S "%s", get_xdir(),
               fnametime);
  }
...
static int log_open_file(char *servname, char *channame, char *netname) {
  char buf[512];
  int fd;
  char *file;
  time_t currenttime;
...
static void log_open(session *sess) {
  ...
  if (!log_error && sess->logfd == -1) {
    char *filename =
        log_create_pathname(sess->server->servername, sess->channel,
                            server_get_network(sess->server, FALSE));
...
/**
 * Converts a given string using the given iconv converter. This is similar to
 * g_convert_with_fallback, except that it is tolerant of sequences in the
 * original input that are invalid even in from_encoding.
 * g_convert_with_fallback fails for such text, whereas this function replaces
 * such a sequence with the fallback string.
 *
 * If len is -1, strlen(text) is used to calculate the length. Do not pass -1 if
 * text is supposed to contain \0 bytes, such as if from_encoding is a
 * multi-byte encoding like UTF-16.
 */
gchar *text_convert_invalid(const gchar *text, gssize len, GIConv converter,
                            const gchar *fallback, gsize *len_out) {
  gchar *result_part;
  ...
  result_part = g_convert_with_iconv(text, len, converter, &invalid_start_pos,
                                     &result_part_len, NULL);
  ...
  /* One or more invalid sequences exist that need to be replaced with the
   * fallback. */
  ...
  for (;;) {
    ...
    /* Convert everything before the position of the invalid sequence. It should
     * be successful. But iconv may not convert everything till
     * invalid_start_pos since the last few bytes may be part of a shift
     * sequence. So get the new bytes_read and use it as the actual
     * invalid_start_pos to handle this.
     *
     * See https://github.com/hexchat/hexchat/issues/1758
     */
    result_part =
        g_convert_with_iconv(current_start, invalid_start_pos, converter,
                             &invalid_start_pos, &result_part_len, NULL);
    g_iconv(converter, NULL, NULL, NULL, NULL);
    ...
    result_part =
        g_convert_with_iconv(current_start, end - current_start, converter,
                             &invalid_start_pos, &result_part_len, NULL);
    g_iconv(converter, NULL, NULL, NULL, NULL);

    if (result_part != NULL) {
      /* The rest of the text converted successfully. Append it and return the
       * whole converted text. */
    ...
    /* The rest of the text didn't convert successfully. invalid_start_pos has
     * the position of the next invalid sequence. */
  }
}

/**
 * Replaces any invalid UTF-8 in the given text with the unicode replacement
 * character.
 */
gchar *text_fixup_invalid_utf8(const gchar *text, gssize len, gsize *len_out) {
  ...
  return text_convert_invalid(text, len, utf8_fixup_converter,
                              unicode_fallback_string, len_out);
#endif
}
...
void PrintText(session *sess, char *text) { PrintTextTimeStamp(sess, text, 0); }
...
/* Consider the following a NOTES file:
   ...
   * Plugins and Perl scripts (when I get round to signaling perl.c) can
   intercept text events and do what they like
   * The default text engine can be config'ed

   By default it should appear *exactly* the same (I'm working hard not to
   change the default style) but if you go into Settings->Edit Event Texts you
   can change the text's. The format is thus:
...
static char *const pevt_privmsg_help[] = {N_("Nickname"), N_("The message"),
                                          N_("Identified text")};

static char *const pevt_capack_help[] = {N_("Server Name"),
                                         N_("Acknowledged Capabilities")};

static char *const pevt_capdel_help[] = {N_("Server Name"),
                                         N_("Removed Capabilities")};

static char *const pevt_caplist_help[] = {N_("Server Name"),
                                          N_("Server Capabilities")};

static char *const pevt_capreq_help[] = {N_("Requested Capabilities")};
...
static char *const pevt_whoisid_help[] = {N_("Nickname"), N_("Message"),
                                          "Numeric"};
...
static char *const pevt_saslauth_help[] = {N_("Username"), N_("Mechanism")};

static char *const pevt_saslresponse_help[] = {N_("Server Name"),
                                               N_("Raw Numeric or Identifier"),
                                               N_("Username"), N_("Message")};

static char *const pevt_servertext_help[] = {N_("Text"), N_("Server Name"),
                                             N_("Raw Numeric or Identifier")};

static char *const pevt_sslmessage_help[] = {N_("Text"), N_("Server Name")};
...
static char *const pevt_sconnect_help[] = {"PID"};

static char *const pevt_generic_nick_help[] = {
    N_("Nickname"), N_("Server Name"), N_("Network")};
...
static char *const pevt_dccfileabort_help[] = {N_("Nickname"), N_("Filename")};
...
static void pevent_load_defaults(void) {
  int i;
...
int pevent_load(char *filename) {
  /* AGL, I've changed this file and pevent_save, could you please take a look
   * at the changes and possibly modify them to suit you
   *      //David H
   */
  char *buf, *ibuf;
  int fd, i = 0, pnt = 0;
...
static void pevent_check_all_loaded(void) {
  int i;

  for (i = 0; i < NUM_XP; i++) {
    if (pntevts_text[i] == NULL) {
      /*printf ("%s\n", te[i].name);
      g_snprintf(out, sizeof(out), "The data for event %s failed to load.
      Reverting to defaults.\nThis may be because a new version of HexChat is
      loading an old config file.\n\nCheck all print event texts are correct",
      evtnames[i]); gtkutil_simpledialog(out); */
      /* make-te.c sets this 128 flag (DON'T call gettext() flag) */
      if (te[i].num_args & 128)
        pntevts_text[i] = g_strdup(te[i].def);
      else
...
/*
        ...
        1) if prefs.hex_text_stripcolor_msg is set, filter all style control
   codes from arguments 2) always strip \010 (ATTR_HIDDEN) from arguments: it is
   only for use in the format string itself
*/
#define ARG_FLAG(argn) (1 << (argn))

void format_event(session *sess, int index, char **args, char *o, gsize sizeofo,
...
void text_emit(int index, session *sess, char *a, char *b, char *c, char *d,
  ...
  unsigned int stripcolor_args =
      (chanopt_is_set(prefs.hex_text_stripcolor_msg, sess->text_strip)
           ? 0xFFFFFFFF
           : 0);
  char tbuf[NICKLEN + 4];

  if (a != NULL && prefs.hex_text_color_nicks &&
  ...
  case XP_TE_DPRIVACTION:
    if (chanopt_is_set(prefs.hex_input_beep_priv, sess->alert_beep) &&
        (!prefs.hex_away_omit_alerts || !sess->server->is_away))
      sound_beep(sess);
    if (chanopt_is_set(prefs.hex_input_flash_priv, sess->alert_taskbar) &&
        (!prefs.hex_away_omit_alerts || !sess->server->is_away))
  ...
  case XP_TE_HCHANMSG:
    if (chanopt_is_set(prefs.hex_input_beep_hilight, sess->alert_beep) &&
        (!prefs.hex_away_omit_alerts || !sess->server->is_away))
      sound_beep(sess);
    if (chanopt_is_set(prefs.hex_input_flash_hilight, sess->alert_taskbar) &&
        (!prefs.hex_away_omit_alerts || !sess->server->is_away))
      fe_flash_window(sess);
    if (sess->alert_tray == SET_ON)
      fe_tray_set_icon(FE_ICON_MESSAGE);
  ...
  case XP_TE_CHANMSG:
    if (chanopt_is_set(prefs.hex_input_beep_chans, sess->alert_beep) &&
        (!prefs.hex_away_omit_alerts || !sess->server->is_away))
      sound_beep(sess);
    if (chanopt_is_set(prefs.hex_input_flash_chans, sess->alert_taskbar) &&
        (!prefs.hex_away_omit_alerts || !sess->server->is_away))
      fe_flash_window(sess);
    if (sess->alert_tray == SET_ON)
      fe_tray_set_icon(FE_ICON_MESSAGE);
...
int text_emit_by_name(char *name, session *sess, time_t timestamp, char *a,
...
void pevent_save(char *fn) {
  ...
  if (!fn)
    fd = hexchat_open_file("pevents.conf", O_CREAT | O_TRUNC | O_WRONLY, 0x180,
                           XOF_DOMODE);
  else
  ...
  if (fd == -1) {
    /*
       ...
       If we get here when X-Chat is closing the fe-message causes a nice & hard
       crash so we have to use perror which doesn't rely on GTK
     */
  ...
  for (i = 0; i < NUM_XP; i++) {
    write(fd, buf, g_snprintf(buf, sizeof(buf), "event_name=%s\n", te[i].name));
    write(fd, buf,
          g_snprintf(buf, sizeof(buf), "event_text=%s\n\n", pntevts_text[i]));
  }
...
void sound_play(const char *file, gboolean quiet) {
  ...
  if (g_access(wavfile, R_OK) == 0) {
    ...
    if (ca_con == NULL) {
      ...
      ca_context_change_props(ca_con, CA_PROP_APPLICATION_ID, "hexchat",
...
#endif
    {
      ...
      if (cmd) {
        char *quoted_wav = g_shell_quote(wavfile);
        buf = g_strdup_printf("%s %s", cmd, quoted_wav);
        hexchat_exec(buf);
        g_free(buf);
        g_free(cmd);
        g_free(quoted_wav);
...
void sound_save() {
  ...
  for (i = 0; i < NUM_XP; i++) {
    if (sound_files[i] && sound_files[i][0]) {
      write(fd, buf, g_snprintf(buf, sizeof(buf), "event=%s\n", te[i].name));
      write(fd, buf,
            g_snprintf(buf, sizeof(buf), "sound=%s\n\n", sound_files[i]));
    }
  }
