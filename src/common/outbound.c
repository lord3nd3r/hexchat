...
static int cmd_server(session *sess, char *tbuf, char *word[],
                      char *word_eol[]);
static void handle_say(session *sess, char *text, int check_spch);

static void notj_msg(struct session *sess) {
...
void process_data_init(char *buf, char *cmd, char *word[], char *word_eol[],
                       gboolean handle_quotes, gboolean allow_escape_quotes) {
  int wordcount = 2;
  int space = FALSE;
  int quote = FALSE;
  int j = 0;
...
/* ADDSERVER <networkname> <serveraddress>, add a new network and server to the
 * network list */
static int cmd_addserver(struct session *sess, char *tbuf, char *word[],
                         char *word_eol[]) {
  ircnet *network;
  ...
  if (*word[2] && *word_eol[3]) {
     ...
     */
      ...
      PrintTextf(sess, _("Server %s already exists on network %s.\n"),
                 word_eol[3], word[2]);
      return TRUE; /* unsuccessful, but the syntax was correct so we don't want
                      to show the help */
    }
...
static int cmd_allchannels(session *sess, char *tbuf, char *word[],
                           char *word_eol[]) {
...
static int cmd_allchannelslocal(session *sess, char *tbuf, char *word[],
                                char *word_eol[]) {
  GSList *list = sess_list;
...
static int cmd_back(struct session *sess, char *tbuf, char *word[],
                    char *word_eol[]) {
  if (sess->server->is_away) {
...
static char *create_mask(session *sess, char *mask, char *mode, char *typestr,
                         int deop) {
  int type;
  struct User *user;
  char *at, *dot, *lastdot;
  ...
  if (user &&
      user->hostname) /* it's a nickname, let's find a proper ban mask */
  {
    if (deop)
      p2 = user->nick;
    else
    ...
    *at = 0;
    ...
    if (mask[0] == '~' || mask[0] == '+' || mask[0] == '=' || mask[0] == '^' ||
      ...
      /* ident might be too long, we just ban what it gives and add an * in the
       * end */
    ...
    *at = '@';
    ...
    buf[0] = 0;
    if (inet_addr(fullhost) !=
        (guint32)-1) /* "fullhost" is really a IP number */
    {
      ...
      *lastdot = '.';
      ...
      case 2:
        g_snprintf(buf, sizeof(buf), "%s %s *!%s@%s.*", mode, p2, username,
                   domain);
        break;
...
static void ban(session *sess, char *tbuf, char *mask, char *bantypestr,
                int deop) {
  char *banmask =
      create_mask(sess, mask, deop ? "-o+b" : "+b", bantypestr, deop);
  server *serv = sess->server;
...
static int cmd_chanopt(struct session *sess, char *tbuf, char *word[],
                       char *word_eol[]) {
  int ret;
...
static int cmd_charset(struct session *sess, char *tbuf, char *word[],
                       char *word_eol[]) {
  server *serv = sess->server;
  int offset = 0;
...
static int cmd_debug(struct session *sess, char *tbuf, char *word[],
  ...
  while (list) {
    ...
    sprintf(tbuf, "%p %1x %-10.10s %-10.10s %-10.10s %p\n", s, s->type,
            s->channel, s->waitchannel, s->willjoinchannel, s->server);
    PrintText(sess, tbuf);
    list = list->next;
  }
...
static int mdehop_cb(struct User *user, multidata *data) {
  if (user->hop && !user->me) {
...
static int mdeop_cb(struct User *user, multidata *data) {
  if (user->op && !user->me) {
    data->nicks[data->i] = user->nick;
    data->i++;
  }
...
static void menu_free(menu_entry *me) {
  g_free(me->path);
  g_free(me->label);
  g_free(me->cmd);
  g_free(me->ucmd);
...
static menu_entry *menu_entry_find(char *path, char *label) {
  GSList *list;
  menu_entry *me;
...
static void menu_del_children(char *path, char *label) {
  GSList *list, *next;
  menu_entry *me;
  char buf[512];
...
static int menu_del(char *path, char *label) {
  GSList *list;
  menu_entry *me;
...
static char menu_is_mainmenu_root(char *path, gint16 *offset) {
...
static void menu_add(char *path, char *label, char *cmd, char *ucmd, int pos,
                     int state, int markup, int enable, int mod, int key,
                     char *group, char *icon) {
  menu_entry *me;
...
static int cmd_menu(struct session *sess, char *tbuf, char *word[],
                    char *word_eol[]) {
  int idx = 2;
  int len;
  int pos = 0xffff;
  ...
  if (!g_ascii_strcasecmp(word[idx], "ADD")) {
    if (toggle) {
      menu_add(tbuf, label, word[idx + 2], word[idx + 3], pos, state, markup,
               enable, mod, key, NULL, NULL);
    } else {
      if (word[idx + 2][0])
        menu_add(tbuf, label, word[idx + 2], NULL, pos, state, markup, enable,
                 mod, key, group, icon);
      else
        menu_add(tbuf, label, NULL, NULL, pos, state, markup, enable, mod, key,
                 group, icon);
    }
    return TRUE;
  }
...
static int mhop_cb(struct User *user, multidata *data) {
  if (!user->hop) {
    data->nicks[data->i] = user->nick;
    data->i++;
  }
...
static int cmd_mhop(struct session *sess, char *tbuf, char *word[],
                    char *word_eol[]) {
...
static int mkick_cb(struct User *user, multidata *data) {
  if (!user->op && !user->me)
    data->sess->server->p_kick(data->sess->server, data->sess->channel,
                               user->nick, data->reason);
  return TRUE;
}

static int mkickops_cb(struct User *user, multidata *data) {
  if (user->op && !user->me)
    data->sess->server->p_kick(data->sess->server, data->sess->channel,
                               user->nick, data->reason);
  return TRUE;
...
static int cmd_doat(struct session *sess, char *tbuf, char *word[],
                    char *word_eol[]) {
  GStrv channels;
  guint i;
...
static void exec_check_process(struct session *sess) {
  int val;
        ...
        if (buf[i] >= 'A' && buf[i] <= 'z') {
          if (buf[i] == 'm') {
            ...
            if (backc) {
              ...
              j += g_snprintf(&nbuf[j], len - j, "\003%d,%02d", colf, colb);
            } else {
              colf = escconv[colf % 14];
              j += g_snprintf(&nbuf[j], len - j, "\003%02d", colf);
            }
          }
        cont:
          esc = FALSE;
...
static void exec_print_line(session *sess, char *data, gssize len,
                            gboolean tochannel) {
  char *valid;
...
static gboolean exec_data(GIOChannel *source, GIOCondition condition,
                          struct nbexec *s) {
...
static void get_bool_cb(int val, getvalinfo *info) {
  char buf[512];
...
static int cmd_getbool(struct session *sess, char *tbuf, char *word[],
                       char *word_eol[]) {
  getvalinfo *info;
...
static int cmd_getint(struct session *sess, char *tbuf, char *word[],
                      char *word_eol[]) {
  getvalinfo *info;
...
static int cmd_getfile(struct session *sess, char *tbuf, char *word[],
                       char *word_eol[]) {
  int idx = 2;
  int flags = 0;
  ...
  fe_get_file(word[idx + 1], word[idx + 2], (void *)get_file_cb,
              g_strdup(word[idx]), flags);
...
static int cmd_getstr(struct session *sess, char *tbuf, char *word[],
                      char *word_eol[]) {
  getvalinfo *info;
...
static int cmd_ghost(struct session *sess, char *tbuf, char *word[],
                     char *word_eol[]) {
  if (!word[2][0])
    return FALSE;
...
static int cmd_gui(struct session *sess, char *tbuf, char *word[],
                   char *word_eol[]) {
  switch (str_ihash(word[2])) {
  case 0x058b836e:
    fe_ctrl_gui(sess, 8, 0);
    break; /* APPLY */
  case 0xac1eee45:
  ...
  case 0x61addbe3:
    ...
    break; /* ICONIFY */
  case 0xc0851aaa:
    fe_message(word[3], FE_MSG_INFO | FE_MSG_MARKUP);
    break; /* MSGBOX */
  case 0x0035dafd:
...
static void show_help_line(session *sess, help_list *hl, char *name,
                           char *usage) {
  int j, len, max;
  char *p;
...
static int cmd_id(struct session *sess, char *tbuf, char *word[],
                  char *word_eol[]) {
  if (word[2][0]) {
...
static int cmd_killall(struct session *sess, char *tbuf, char *word[],
                       char *word_eol[]) {
  hexchat_exit();
  return 2;
}
...
static int cmd_load(struct session *sess, char *tbuf, char *word[],
  ...
  sprintf(tbuf,
          "Unknown file type %s. Maybe you need to install the Perl or Python "
          "plugin?\n",
          word[2]);
  PrintText(sess, tbuf);
#endif
...
char *split_up_text(struct session *sess, char *text, int cmd_length,
                    char *split_text) {
...
static int cmd_me(struct session *sess, char *tbuf, char *word[],
  ...
  if (dcc_write_chat(sess->channel, tbuf)) {
    ...
    if (sess->server->connected) {
      while ((split_text =
                  split_up_text(sess, act + offset, cmd_length, split_text))) {
        sess->server->p_action(sess->server, sess->channel, split_text);
        /* print it to screen */
        inbound_action(sess, sess->channel, sess->server->nick, "", split_text,
                       TRUE, FALSE, &no_tags);
      ...
      /* print it to screen */
      inbound_action(sess, sess->channel, sess->server->nick, "", act + offset,
                     TRUE, FALSE, &no_tags);
    } else {
      notc_msg(sess);
    }
...
static int cmd_mode(struct session *sess, char *tbuf, char *word[],
                    char *word_eol[]) {
  ...
  if ((sess->type != SESS_CHANNEL && sess->type != SESS_DIALOG) ||
      (!(*word[2] == '-' || *word[2] == '+' || *word[2] == '\0') &&
       (is_channel(sess->server, word[2]) ||
        !rfc_casecmp(sess->server->nick, word[2])))) {
...
static int mop_cb(struct User *user, multidata *data) {
  if (!user->op) {
    data->nicks[data->i] = user->nick;
    data->i++;
  }
...
static int cmd_mop(struct session *sess, char *tbuf, char *word[],
                   char *word_eol[]) {
  ...
  if (*nick) {
    if (*msg) {
      if (strcmp(nick, ".") == 0) { /* /msg the last nick /msg'ed */
        ...
        safe_strcpy(sess->lastnick, nick,
                    NICKLEN); /* prime the last nick memory */
      }
      ...
      if (*nick == '=') {
        ...
        while ((split_text = split_up_text(sess, msg + offset, cmd_length,
                                           split_text))) {
...
static int cmd_nick(struct session *sess, char *tbuf, char *word[],
  ...
  if (*nick) {
    if (sess->server->connected)
      ...
      message_tags_data no_tags = MESSAGE_TAGS_DATA_INIT;
      inbound_newnick(sess->server, sess->server->nick, nick, TRUE, &no_tags);
    }
    return TRUE;
  }
  return FALSE;
...
static int cmd_notice(struct session *sess, char *tbuf, char *word[],
  ...
  if (*word[2] && *word_eol[3]) {
    while ((split_text =
                split_up_text(sess, text + offset, cmd_length, split_text))) {
...
static int cmd_query(struct session *sess, char *tbuf, char *word[],
  ...
  if (*nick && !is_channel(sess->server, nick)) {
    ...
    if (*msg) {
      ...
      while ((split_text =
                  split_up_text(sess, msg + offset, cmd_length, split_text))) {
        sess->server->p_message(sess->server, nick, split_text);
...
static int cmd_quiet(struct session *sess, char *tbuf, char *word[],
                     char *word_eol[]) {
  char *quietmask;
  server *serv = sess->server;
...
static int cmd_unquiet(struct session *sess, char *tbuf, char *word[],
                       char *word_eol[]) {
  /* Allow more than one mask in /unban -- tvk */
  int i = 2;
...
static int cmd_recv(struct session *sess, char *tbuf, char *word[],
                    char *word_eol[]) {
  if (*word_eol[2]) {
...
static int cmd_send(struct session *sess, char *tbuf, char *word[],
                    char *word_eol[]) {
  guint32 addr;
  socklen_t len;
...
static int cmd_setcursor(struct session *sess, char *tbuf, char *word[],
                         char *word_eol[]) {
  int delta = FALSE;
...
static int cmd_settext(struct session *sess, char *tbuf, char *word[],
                       char *word_eol[]) {
  fe_set_inputbox_contents(sess, word_eol[2]);
  return TRUE;
}

static int cmd_splay(struct session *sess, char *tbuf, char *word[],
                     char *word_eol[]) {
  if (*word[2]) {
...
static int parse_irc_url(char *url, char *server_name[], char *port[],
                         char *channel[], char *key[], int *use_ssl) {
  char *co;
#ifdef USE_OPENSSL
...
static int cmd_server(struct session *sess, char *tbuf, char *word[],
  ...
  if (!parse_irc_url(word[2 + offset], &server_name, &port, &channel, &key,
                     &use_ssl)) {
    is_url = FALSE;
...
#ifdef USE_OPENSSL
  if (g_strcmp0(word[2], "-ssl") == 0 ||
      g_strcmp0(word[2], "-ssl-noverify") == 0 ||
      g_strcmp0(word[2], "-insecure") == 0)
    offset++;
#endif
...
static int cmd_tray(struct session *sess, char *tbuf, char *word[],
                    char *word_eol[]) {
...
static int cmd_unload(struct session *sess, char *tbuf, char *word[],
                      char *word_eol[]) {
#ifdef USE_PLUGIN
...
static int cmd_reload(struct session *sess, char *tbuf, char *word[],
                      char *word_eol[]) {
#ifdef USE_PLUGIN
  gboolean by_file = FALSE;
...
static void url_join_only(server *serv, char *tbuf, char *channel, char *key) {
...
static int cmd_url(struct session *sess, char *tbuf, char *word[],
                   char *word_eol[]) {
  if (word[2][0]) {
    char *server_name = NULL;
    char *port = NULL;
...
static int userlist_cb(struct User *user, session *sess) {
  time_t lt;
  ...
  PrintTextf(sess,
             "\00306%s\t\00314[\00310%-38s\00314] \017ov\0033=\017%d%d away=%u "
             "lt\0033=\017%ld\n",
             user->nick, user->hostname, user->op, user->voice, user->away,
             (long)lt);

  return TRUE;
}

static int cmd_uselect(struct session *sess, char *tbuf, char *word[],
                       char *word_eol[]) {
  int idx = 2;
  int clear = TRUE;
  int scroll = FALSE;
...
static int wallchop_cb(struct User *user, multidata *data) {
  if (user->op) {
    if (data->i)
      strcat(data->tbuf, ",");
  ...
  if (data->i == 5) {
    data->i = 0;
    sprintf(data->tbuf + strlen(data->tbuf), " :[@%s] %s", data->sess->channel,
            data->reason);
...
const struct commands xc_cmds[] = {
    ...
    {"ADDSERVER", cmd_addserver, 0, 0, 1,
     N_("ADDSERVER <NewNetwork> <hostname/port>, adds a new network with a new "
    ...
    {"ALLCHANL", cmd_allchannelslocal, 0, 0, 1,
     N_("ALLCHANL <cmd>, sends a command to all channels on the current "
        "server")},
    {"ALLSERV", cmd_allservers, 0, 0, 1,
     N_("ALLSERV <cmd>, sends a command to all servers you're in")},
    {"AWAY", cmd_away, 1, 0, 1,
     N_("AWAY [<reason>], sets you away (use /BACK to unset)")},
    {"BACK", cmd_back, 1, 0, 1, N_("BACK, sets you back (not away)")},
    {"BAN", cmd_ban, 1, 1, 1,
     N_("BAN <mask> [<bantype>], bans everyone matching the mask from the "
        "current channel. If they are already on the channel this doesn't kick "
        "them (needs chanop)")},
    {"CHANOPT", cmd_chanopt, 0, 0, 1,
     N_("CHANOPT [-quiet] <variable> [<value>]")},
    {"CHARSET", cmd_charset, 0, 0, 1,
     N_("CHARSET [<encoding>], get or set the encoding used for the current "
        "connection")},
    {"CLEAR", cmd_clear, 0, 0, 1,
     N_("CLEAR [ALL|HISTORY|[-]<amount>], Clears the current text window or "
        "command history")},
    {"CLOSE", cmd_close, 0, 0, 1,
     N_("CLOSE [-m], Closes the current tab, closing the window if this is the "
        "only open tab, or with the \"-m\" flag, closes all queries.")},

    {"COUNTRY", cmd_country, 0, 0, 1,
     N_("COUNTRY [-s] <code|wildcard>, finds a country code, eg: au = "
        "australia")},
    {"CTCP", cmd_ctcp, 1, 0, 1,
     N_("CTCP <nick> <message>, send the CTCP message to nick, common messages "
        "are VERSION and USERINFO")},
    {"CYCLE", cmd_cycle, 1, 1, 1,
     N_("CYCLE [<channel>], parts the current or given channel and immediately "
        "rejoins")},
    {"DCC", cmd_dcc, 0, 0, 1,
     N_("\n"
        ...
        "DCC PCHAT <nick>                    - offer DCC CHAT using passive "
        "mode\n"
    ...
    {"DEHOP", cmd_dehop, 1, 1, 1,
     N_("DEHOP <nick>, removes chanhalf-op status from the nick on the current "
        "channel (needs chanop)")},
    {"DELBUTTON", cmd_delbutton, 0, 0, 1,
     N_("DELBUTTON <name>, deletes a button from under the user-list")},
    {"DEOP", cmd_deop, 1, 1, 1,
     N_("DEOP <nick>, removes chanop status from the nick on the current "
        "channel (needs chanop)")},
    {"DEVOICE", cmd_devoice, 1, 1, 1,
     N_("DEVOICE <nick>, removes voice status from the nick on the current "
        "channel (needs chanop)")},
    {"DISCON", cmd_discon, 0, 0, 1, N_("DISCON, Disconnects from server")},
    {"DNS", cmd_dns, 0, 0, 1,
     N_("DNS <nick|host|ip>, Resolves an IP or hostname")},
    {"DOAT", cmd_doat, 0, 0, 1, N_("DOAT <channel,list,/network> <command>")},
    {"ECHO", cmd_echo, 0, 0, 1, N_("ECHO <text>, Prints text locally")},
#ifndef WIN32
    {"EXEC", cmd_exec, 0, 0, 1,
     N_("EXEC [-o] <command>, runs the command. If -o flag is used then output "
    ...
    {"EXECKILL", cmd_execk, 0, 0, 1,
     N_("EXECKILL [-9], kills a running exec in the current session. If -9 is "
        "given the process is SIGKILL'ed")},
#ifndef __EMX__
    {"EXECSTOP", cmd_execs, 0, 0, 1, N_("EXECSTOP, sends the process SIGSTOP")},
    {"EXECWRITE", cmd_execw, 0, 0, 1,
     N_("EXECWRITE [-q|--], sends data to the processes stdin; use -q flag to "
        "quiet/suppress output at text box; use -- flag to stop interpreting "
        "arguments as flags, needed if -q itself would occur as data")},
#endif
#endif
#if 0
    ...
    {"GETFILE", cmd_getfile, 0, 0, 1,
     "GETFILE [-folder] [-multi] [-save] <command> <title> [<initial>]"},
    {"GETINT", cmd_getint, 0, 0, 1, "GETINT <default> <command> <prompt>"},
    {"GETSTR", cmd_getstr, 0, 0, 1, "GETSTR <default> <command> <prompt>"},
    {"GHOST", cmd_ghost, 1, 0, 1,
     N_("GHOST <nick> [password], Kills a ghosted nickname")},
    {"GUI", cmd_gui, 0, 0, 1,
     "GUI [APPLY|ATTACH|DETACH|SHOW|HIDE|FOCUS|FLASH|ICONIFY]\n"
    ...
    {"ID", cmd_id, 1, 0, 1,
     N_("ID <password>, identifies yourself to nickserv")},
    ...
    {"INVITE", cmd_invite, 1, 0, 1,
     N_("INVITE <nick> [<channel>], invites someone to a channel, by default "
        "the current channel (needs chanop)")},
    {"JOIN", cmd_join, 1, 0, 0, N_("JOIN <channel>, joins the channel")},
    {"KICK", cmd_kick, 1, 1, 1,
     N_("KICK <nick> [reason], kicks the nick from the current channel (needs "
        "chanop)")},
    {"KICKBAN", cmd_kickban, 1, 1, 1,
     N_("KICKBAN <nick> [reason], bans then kicks the nick from the current "
        "channel (needs chanop)")},
    {"KILLALL", cmd_killall, 0, 0, 1, "KILLALL, immediately exit"},
    {"LAGCHECK", cmd_lagcheck, 0, 0, 1, N_("LAGCHECK, forces a new lag check")},
    {"LASTLOG", cmd_lastlog, 0, 0, 1,
     N_("LASTLOG [-h] [-m] [-r] [--] <string>, searches for a string in the "
        "buffer\n"
        "    Use -h to highlight the found string(s)\n"
        "    Use -m to match case\n"
        "    Use -r when string is a Regular Expression\n"
        "    Use -- (double hyphen) to end options when searching for, say, "
        "the string '-r'")},
    {"LIST", cmd_list, 1, 0, 1, 0},
    {"LOAD", cmd_load, 0, 0, 1,
     N_("LOAD [-e] <file>, loads a plugin or script")},

    {"MDEHOP", cmd_mdehop, 1, 1, 1,
     N_("MDEHOP, Mass deop's all chanhalf-ops in the current channel (needs "
        "chanop)")},
    {"MDEOP", cmd_mdeop, 1, 1, 1,
     N_("MDEOP, Mass deop's all chanops in the current channel (needs "
        "chanop)")},
    {"ME", cmd_me, 0, 0, 1,
     N_("ME <action>, sends the action to the current channel (actions are "
        "written in the 3rd person, like /me jumps)")},
    {"MENU", cmd_menu, 0, 0, 1,
     "MENU [-eX] [-i<ICONFILE>] [-k<mod>,<key>] [-m] [-pX] [-r<X,group>] [-tX] "
     "{ADD|DEL} <path> [command] [unselect command]\n"
     "       See "
     "http://hexchat.readthedocs.org/en/latest/"
     "plugins.html#controlling-the-gui for more details."},
    {"MHOP", cmd_mhop, 1, 1, 1,
     N_("MHOP, Mass hop's all users in the current channel (needs chanop)")},
    {"MKICK", cmd_mkick, 1, 1, 1,
     N_("MKICK, Mass kicks everyone except you in the current channel (needs "
        "chanop)")},
    {"MODE", cmd_mode, 1, 0, 1, 0},
    {"MOP", cmd_mop, 1, 1, 1,
     N_("MOP, Mass op's all users in the current channel (needs chanop)")},
    {"MSG", cmd_msg, 0, 0, 1,
     N_("MSG <nick> <message>, sends a private message, message \".\" to send "
        "to last nick or prefix with \"=\" for dcc chat")},
    ...
    {"NEWSERVER", cmd_newserver, 0, 0, 1,
     N_("NEWSERVER [-noconnect] <hostname> [<port>]")},
    ...
    {"NOTIFY", cmd_notify, 0, 0, 1,
     N_("NOTIFY [-n network1[,network2,...]] [<nick>], displays your notify "
        "list or adds someone to it")},
    {"OP", cmd_op, 1, 1, 1,
     N_("OP <nick>, gives chanop status to the nick (needs chanop)")},
    {"PART", cmd_part, 1, 1, 0,
     N_("PART [<channel>] [<reason>], leaves the channel, by default the "
        "current one")},
    {"PING", cmd_ping, 1, 0, 1,
     N_("PING <nick | channel>, CTCP pings nick or channel")},
    {"QUERY", cmd_query, 0, 0, 1,
     N_("QUERY [-nofocus] <nick> [message], opens up a new privmsg window to "
        "someone and optionally sends a message")},
    {"QUIET", cmd_quiet, 1, 1, 1,
     N_("QUIET <mask> [<quiettype>], quiet everyone matching the mask in the "
        "current channel if supported by the server.")},
    {"QUIT", cmd_quit, 0, 0, 1,
    ...
    {"RECONNECT", cmd_reconnect, 0, 0, 1,
     N_("RECONNECT [-ssl|-ssl-noverify] [<host>] [<port>] [<password>], Can be "
        "called just as /RECONNECT to reconnect to the current server or with "
        "/RECONNECT ALL to reconnect to all the open servers")},
#else
    {"RECONNECT", cmd_reconnect, 0, 0, 1,
     N_("RECONNECT [<host>] [<port>] [<password>], Can be called just as "
        "/RECONNECT to reconnect to the current server or with /RECONNECT ALL "
        "to reconnect to all the open servers")},
#endif
    {"RECV", cmd_recv, 1, 0, 1,
     N_("RECV <text>, send raw data to HexChat, as if it was received from the "
        "IRC server")},
    {"RELOAD", cmd_reload, 0, 0, 1,
     N_("RELOAD <name>, reloads a plugin or script")},
    {"SAY", cmd_say, 0, 0, 1,
     N_("SAY <text>, sends the text to the object in the current window")},
    {"SEND", cmd_send, 0, 0, 1, N_("SEND <nick> [<file>]")},
#ifdef USE_OPENSSL
    {"SERVCHAN", cmd_servchan, 0, 0, 1,
     N_("SERVCHAN [-insecure|-ssl|-ssl-noverify] <host> <port> <channel>, "
        "connects and joins a channel using ssl unless otherwise specified")},
#else
    {"SERVCHAN", cmd_servchan, 0, 0, 1,
    ...
    {"SERVER", cmd_server, 0, 0, 1,
     N_("SERVER [-insecure|-ssl|-ssl-noverify] <host> [<port>] [<password>], "
        "connects to a server using ssl unless otherwise specified, the "
        "default port is 6697 for ssl connections and 6667 for insecure "
        "connections")},
#else
    {"SERVER", cmd_server, 0, 0, 1,
     N_("SERVER <host> [<port>] [<password>], connects to a server, the "
        "default port is 6667")},
#endif
    {"SET", cmd_set, 0, 0, 1,
     N_("SET [-e] [-off|-on] [-quiet] <variable> [<value>]")},
    {"SETCURSOR", cmd_setcursor, 0, 0, 1,
     N_("SETCURSOR [-|+]<position>, reposition the cursor in the inputbox")},
    {"SETTAB", cmd_settab, 0, 0, 1,
     N_("SETTAB <new name>, change a tab's name, tab_trunc limit still "
        "applies")},
    {"SETTEXT", cmd_settext, 0, 0, 1,
     N_("SETTEXT <new text>, replace the text in the input box")},
    {"SPLAY", cmd_splay, 0, 0, 1, "SPLAY <soundfile>"},
    {"TOPIC", cmd_topic, 1, 1, 1,
     N_("TOPIC [<topic>], sets the topic if one is given, else shows the "
        "current topic")},
    {"TRAY", cmd_tray, 0, 0, 1,
    ...
    {"UNLOAD", cmd_unload, 0, 0, 1,
     N_("UNLOAD <name>, unloads a plugin or script")},
    {"UNQUIET", cmd_unquiet, 1, 1, 1,
     N_("UNQUIET <mask> [<mask>...], unquiets the specified masks if supported "
        "by the server.")},
    {"URL", cmd_url, 0, 0, 1, N_("URL <url>, opens a URL in your browser")},
    {"USELECT", cmd_uselect, 0, 1, 0,
     N_("USELECT [-a] [-s] <nick1> <nick2> etc, highlights nick(s) in channel "
        "userlist")},
    ...
    {"WALLCHOP", cmd_wallchop, 1, 1, 1,
     N_("WALLCHOP <message>, sends the message to all chanops on the current "
        "channel")},
    {0, 0, 0, 0, 0, 0}};
...
static struct commands *find_internal_command(char *name) {
  ...
  return bsearch(name, xc_cmds, (sizeof(xc_cmds) / sizeof(xc_cmds[0])) - 1,
                 sizeof(xc_cmds[0]), command_compare);
}

static gboolean usercommand_show_help(session *sess, char *name) {
  struct popup *pop;
...
int auto_insert(char *dest, gsize destlen, unsigned char *src, char *word[],
    ...
    if (src[0] == '%' || src[0] == '&') {
      if (isdigit((unsigned char)src[1])) {
        ...
        case 'y':
          ...
          g_snprintf(buf, sizeof(buf), "%4d%02d%02d", 1900 + tm_ptr->tm_year,
...
void check_special_chars(char *cmd, int do_ascii) /* check for %X */
{
  ...
  while (cmd[j]) {
    ...
    case '%':
      occur++;
      if (do_ascii && j + 3 < len &&
          (isdigit((unsigned char)cmd[j + 1]) &&
           isdigit((unsigned char)cmd[j + 2]) &&
...
static int nick_comp_cb(struct User *user, nickdata *data) {
  int lenu;
...
static void perform_nick_completion(struct session *sess, char *cmd,
                                    char *tbuf) {
  int len;
...
static void user_command(session *sess, char *tbuf, char *cmd, char *word[],
                         char *word_eol[]) {
...
static void handle_say(session *sess, char *text, int check_spch) {
  ...
  if (!sess->channel[0] || sess->type == SESS_SERVER ||
      sess->type == SESS_NOTICES || sess->type == SESS_SNOTICES) {
    notj_msg(sess);
    goto xit;
  }
  ...
  if (sess->type == SESS_DIALOG) {
    ...
    if (dcc) {
      inbound_chanmsg(sess->server, NULL, sess->channel, sess->server->nick,
                      text, TRUE, FALSE, &no_tags);
...
char *command_insert_vars(session *sess, char *cmd) {
  ...
  {
    return g_strdup(
        cmd); /* the return value will be freed so we must srtdup() it */
  }
  ...
  g_string_append(expanded,
                  cmd); /* copy any remaining string after the last '%' */
...
static int handle_user_input(session *sess, char *text, int history,
                             int nocommand) {
  if (*text == '\0')
    return 1;
