 ...
 * Bernhard Valenti <bernhard.valenti@gmx.net> 2000-11-21: Fixed DCC send behind
 * nat
 *
...
/* Setting _FILE_OFFSET_BITS to 64 doesn't change lseek to use off64_t on
 * Windows, so override lseek to the version that does */
...
struct dccstat_info dccstat[] = {
    {N_("Waiting"), 1 /*black */}, {N_("Active"), 12 /*cyan */},
    {N_("Failed"), 4 /*red */},    {N_("Done"), 3 /*green */},
    {N_("Connect"), 1 /*black */}, {N_("Aborted"), 4 /*red */},
};
...
static gboolean dcc_read_ack(GIOChannel *source, GIOCondition condition,
                             struct DCC *dcc);
...
static double timeval_diff(GTimeVal *greater, GTimeVal *less) {
  long usecdiff;
  double result;
...
static void dcc_unthrottle(struct DCC *dcc) {
...
static void dcc_calc_cps(struct DCC *dcc) {
  GTimeVal now;
  gint64 oldcps;
  ...
  if (!dcc->firstcpstv.tv_sec && !dcc->firstcpstv.tv_usec)
    ...
    oldcps = dcc->cps;
    dcc->cps = (gint64)((posdiff / timediff) * (timediff / startdiff) +
                        dcc->cps * (1.0 - (timediff / startdiff)));
...
static void dcc_remove_from_sum(struct DCC *dcc) {
...
gboolean is_dcc_completed(struct DCC *dcc) {
  if (dcc != NULL)
    return (dcc->dccstat == STAT_FAILED || dcc->dccstat == STAT_DONE ||
        ...
        if (prefs.hex_dcc_stall_timeout > 0) {
          if (!dcc->throttled &&
            ...
            EMIT_SIGNAL(XP_TE_DCCSTALL, dcc->serv->front_session,
                        dcctypes[dcc->type], file_part(dcc->file), dcc->nick,
                        NULL, 0);
...
static int dcc_lookup_proxy(char *host, struct sockaddr_in *addr) {
  struct hostent *h;
...
#define DCC_USE_PROXY()                                                        \
  (prefs.hex_net_proxy_host[0] && prefs.hex_net_proxy_type > 0 &&              \
   prefs.hex_net_proxy_type < 5 && prefs.hex_net_proxy_use != 1)
...
void dcc_abort(session *sess, struct DCC *dcc) {
  if (dcc) {
    ...
    case STAT_ACTIVE:
      ...
      case TYPE_CHATSEND:
      case TYPE_CHATRECV:
        EMIT_SIGNAL(XP_TE_DCCCHATABORT, sess, dcc->nick, NULL, NULL, NULL, 0);
        break;
      case TYPE_SEND:
        EMIT_SIGNAL(XP_TE_DCCSENDABORT, sess, dcc->nick, file_part(dcc->file),
                    NULL, NULL, 0);
        break;
      case TYPE_RECV:
        EMIT_SIGNAL(XP_TE_DCCRECVABORT, sess, dcc->nick, dcc->file, NULL, NULL,
                    0);
      }
      break;
    default:
...
static int dcc_chat_line(struct DCC *dcc, char *line) {
  session *sess;
  char *word[PDIWORDS];
  char *po;
  int ret, i;
  ...
  line = text_convert_invalid(line, -1, dcc->serv->read_converter,
                              unicode_fallback_string, NULL);
...
static gboolean dcc_read_chat(GIOChannel *source, GIOCondition condition,
                              struct DCC *dcc) {
  int i, len, dead;
  char portbuf[32];
  char lbuf[2050];
  ...
  while (1) {
    ...
    if (!dcc->iotag)
      dcc->iotag =
          fe_input_add(dcc->sok, FIA_READ | FIA_EX, dcc_read_chat, dcc);
    ...
    while (i < len) {
      ...
      case '\n':
        ...
        if (dead ||
            !dcc->dccchat) /* the dcc has been closed, don't use (DCC *)! */
          return TRUE;
...
static void dcc_calc_average_cps(struct DCC *dcc) {
  time_t sec;
...
static void dcc_send_ack(struct DCC *dcc) {
  /* send in 32-bit big endian */
  guint32 pos = htonl(dcc->pos & 0xffffffff);
  send(dcc->sok, (char *)&pos, 4, 0);
}

static gboolean dcc_read(GIOChannel *source, GIOCondition condition,
                         struct DCC *dcc) {
  char *old;
  char buf[4096];
  int n;
  ...
  if (dcc->fp == -1) {
    ...
    if (dcc->resumable) {
      gchar *filename_fs =
          g_filename_from_utf8(dcc->destfile, -1, NULL, NULL, NULL);
      ...
      if (g_access(dcc->destfile, F_OK) == 0) {
        ...
        EMIT_SIGNAL(XP_TE_DCCRENAME, dcc->serv->front_session, old,
                    dcc->destfile, NULL, NULL, 0);
        g_free(old);
      }

      filename_fs = g_filename_from_utf8(dcc->destfile, -1, NULL, NULL, NULL);
      dcc->fp = g_open(filename_fs, OFLAGS | O_TRUNC | O_WRONLY | O_CREAT,
  ...
  while (1) {
    ...
    if (dcc->pos >= dcc->size) {
      ...
      dcc_calc_average_cps(
          dcc); /* this must be done _after_ dcc_close, or dcc_remove_from_sum
                   will see the wrong value in dcc->cps */
...
static void dcc_open_query(server *serv, char *nick) {
  if (prefs.hex_gui_autoopen_dialog)
    open_query(serv, nick, FALSE);
}

static gboolean dcc_did_connect(GIOChannel *source, GIOCondition condition,
                                struct DCC *dcc) {
  int er;
  ...
  if (condition & G_IO_ERR) {
    ...
    EMIT_SIGNAL(XP_TE_DCCCONFAIL, dcc->serv->front_session, dcctypes[dcc->type],
                dcc->nick, errorstring(er), NULL, 0);
  ...
  if (connect(dcc->sok, (struct sockaddr *)&addr, sizeof(addr)) != 0) {
    ...
    if (er != EISCONN) {
      EMIT_SIGNAL(XP_TE_DCCCONFAIL, dcc->serv->front_session,
                  dcctypes[dcc->type], dcc->nick, errorstring(er), NULL, 0);
      dcc->dccstat = STAT_FAILED;
      fe_dcc_update(dcc);
      return FALSE;
    }
...
static gboolean dcc_connect_finished(GIOChannel *source, GIOCondition condition,
                                     struct DCC *dcc) {
  char host[128];
  ...
  case TYPE_RECV:
    dcc->iotag = fe_input_add(dcc->sok, FIA_READ | FIA_EX, dcc_read, dcc);
    EMIT_SIGNAL(XP_TE_DCCCONRECV, dcc->serv->front_session, dcc->nick, host,
                dcc->file, NULL, 0);
    break;
  case TYPE_SEND:
    /* passive send */
    ...
    EMIT_SIGNAL(XP_TE_DCCCONSEND, dcc->serv->front_session, dcc->nick, host,
                dcc->file, NULL, 0);
    break;
  ...
  case TYPE_CHATRECV: /* normal chat */
    ...
    EMIT_SIGNAL(XP_TE_DCCCONCHAT, dcc->serv->front_session, dcc->nick, host,
                NULL, NULL, 0);
    break;
  }
...
static gboolean read_proxy(struct DCC *dcc) {
...
static gboolean write_proxy(struct DCC *dcc) {
  struct proxy_state *proxy = dcc->proxy;
  while (proxy->bufferused < proxy->buffersize) {
...
static gboolean proxy_read_line(struct DCC *dcc) {
  struct proxy_state *proxy = dcc->proxy;
  while (1) {
...
static gboolean dcc_wingate_proxy_traverse(GIOChannel *source,
...
static gboolean dcc_socks_proxy_traverse(GIOChannel *source,
                                         GIOCondition condition,
                                         struct DCC *dcc) {
  struct proxy_state *proxy = dcc->proxy;
...
static gboolean dcc_socks5_proxy_traverse(GIOChannel *source,
                                          GIOCondition condition,
                                          struct DCC *dcc) {
  struct proxy_state *proxy = dcc->proxy;
  int auth = prefs.hex_net_proxy_auth && prefs.hex_net_proxy_user[0] &&
             prefs.hex_net_proxy_pass[0];
  ...
  if (proxy->phase == 2) {
    ...
    if (auth) {
      ...
      if (proxy->buffer[0] != 5 ||
          proxy->buffer[1] != 2) /* UPA not supported by server */
      {
        PrintText(dcc->serv->front_session,
...
static gboolean dcc_http_proxy_traverse(GIOChannel *source,
                                        GIOCondition condition,
                                        struct DCC *dcc) {
  struct proxy_state *proxy = dcc->proxy;
  ...
  if (proxy->phase == 1) {
    ...
    proxy->bufferused = 0;
    dcc->iotag =
        fe_input_add(dcc->sok, FIA_READ | FIA_EX, dcc_http_proxy_traverse, dcc);
    ++proxy->phase;
  }
  ...
  if (proxy->phase == 2) {
    if (!proxy_read_line(dcc))
      return TRUE;
    /* "HTTP/1.0 200 OK" */
    if (proxy->bufferused < 12 || memcmp(proxy->buffer, "HTTP/", 5) ||
        memcmp(proxy->buffer + 9, "200", 3)) {
      fe_input_remove(dcc->iotag);
      dcc->iotag = 0;
...
static gboolean dcc_proxy_connect(GIOChannel *source, GIOCondition condition,
                                  struct DCC *dcc) {
  fe_input_remove(dcc->iotag);
  dcc->iotag = 0;
  ...
  case 1:
    return dcc_wingate_proxy_traverse(source, condition, dcc);
  case 2:
...
static void dcc_connect(struct DCC *dcc) {
  int ret;
  char tbuf[400];
  ...
  if (dcc->pasvid && dcc->port == 0) {
    ...
    if (dcc->type == TYPE_RECV)
      g_snprintf(tbuf, sizeof(tbuf),
                 strchr(dcc->file, ' ')
                     ...
                     : "DCC SEND %s %u %d %" G_GUINT64_FORMAT " %d",
                 dcc->file, dcc->addr, dcc->port, dcc->size, dcc->pasvid);
    else
      g_snprintf(tbuf, sizeof(tbuf), "DCC CHAT chat %u %d %d", dcc->addr,
                 dcc->port, dcc->pasvid);
    ...
    if (DCC_USE_PROXY())
      dcc->iotag =
          fe_input_add(dcc->sok, FIA_WRITE | FIA_EX, dcc_proxy_connect, dcc);
    else
      dcc->iotag =
          fe_input_add(dcc->sok, FIA_WRITE | FIA_EX, dcc_connect_finished, dcc);
  }

  fe_dcc_update(dcc);
}

static gboolean dcc_send_data(GIOChannel *source, GIOCondition condition,
                              struct DCC *dcc) {
  char *buf;
  ...
  abortit:
    ...
    EMIT_SIGNAL(XP_TE_DCCSENDFAIL, dcc->serv->front_session,
                file_part(dcc->file), dcc->nick, errorstring(sock_error()),
                NULL, 0);
    dcc_close(dcc, STAT_FAILED, FALSE);
    return TRUE;
  }
...
static gboolean dcc_handle_new_ack(struct DCC *dcc) {
  guint32 ack;
  char buf[32];
  gboolean done = FALSE;
...
static gboolean dcc_accept(GIOChannel *source, GIOCondition condition,
                           struct DCC *dcc) {
  char host[128];
...
static int dcc_listen_init(struct DCC *dcc, session *sess) {
  guint32 my_addr;
...
static void dcc_send_wild(char *file) {
  dcc_send(dccsess, dccto, file, dccmaxcps, 0);
}

void dcc_send(struct session *sess, char *to, char *filename, gint64 maxcps,
              int passive) {
  char outbuf[512];
  GFileInfo *file_info;
  GFile *file;
  ...
  if (*file_part(filename) == '\0' || file_type == G_FILE_TYPE_DIRECTORY ||
      file_size <= 0) {
  ...
  if (passive || dcc_listen_init(dcc, sess)) {
    ...
    if (passive) {
      ...
      g_snprintf(outbuf, sizeof(outbuf),
                 (havespaces) ? "DCC SEND \"%s\" 199 0 %" G_GUINT64_FORMAT " %d"
                              : "DCC SEND %s 199 0 %" G_GUINT64_FORMAT " %d",
                 file_part(dcc->file), dcc->size, dcc->pasvid);
    } else {
      g_snprintf(outbuf, sizeof(outbuf),
                 (havespaces) ? "DCC SEND \"%s\" %u %d %" G_GUINT64_FORMAT
                              : "DCC SEND %s %u %d %" G_GUINT64_FORMAT,
                 file_part(dcc->file), dcc->addr, dcc->port, dcc->size);
    }
    sess->server->p_ctcp(sess->server, to, outbuf);

    EMIT_SIGNAL(XP_TE_DCCOFFER, sess, file_part(dcc->file), to, dcc->file, NULL,
                0);
  } else {
    dcc_close(dcc, 0, TRUE);
  }
...
static struct DCC *find_dcc_from_id(int id, int type) {
  struct DCC *dcc;
  GSList *list = dcc_list;
  while (list) {
    dcc = (struct DCC *)list->data;
    if (dcc->pasvid == id && dcc->dccstat == STAT_QUEUED && dcc->type == type)
      return dcc;
    list = list->next;
  }
  return NULL;
...
static struct DCC *find_dcc_from_port(int port, int type) {
  ...
  while (list) {
    dcc = (struct DCC *)list->data;
    if (dcc->port == port && dcc->dccstat == STAT_QUEUED && dcc->type == type)
      return dcc;
    list = list->next;
  }
  return NULL;
...
void dcc_change_nick(struct server *serv, char *oldnick, char *newnick) {
  struct DCC *dcc;
  GSList *list = dcc_list;
...
static gboolean is_same_file(struct DCC *dcc, struct DCC *new_dcc) {
  ...
  file_info = g_file_query_info(
      file, G_FILE_ATTRIBUTE_ID_FILE "," G_FILE_ATTRIBUTE_ID_FILESYSTEM,
      G_FILE_QUERY_INFO_NONE, NULL, NULL);
  if (file_info == NULL) {
    goto exit;
  }

  new_file_info = g_file_query_info(
      new_file, G_FILE_ATTRIBUTE_ID_FILE "," G_FILE_ATTRIBUTE_ID_FILESYSTEM,
      G_FILE_QUERY_INFO_NONE, NULL, NULL);
  ...
  file_id =
      g_file_info_get_attribute_as_string(file_info, G_FILE_ATTRIBUTE_ID_FILE);
  new_file_id = g_file_info_get_attribute_as_string(new_file_info,
                                                    G_FILE_ATTRIBUTE_ID_FILE);

  filesystem_id =
      g_file_info_get_attribute_as_string(file_info, G_FILE_ATTRIBUTE_ID_FILE);
  new_filesystem_id = g_file_info_get_attribute_as_string(
      new_file_info, G_FILE_ATTRIBUTE_ID_FILE);

  if (file_id != NULL && new_file_id != NULL && filesystem_id != NULL &&
      new_filesystem_id != NULL && strcmp(file_id, new_file_id) == 0 &&
      strcmp(filesystem_id, new_filesystem_id) == 0) {
    result = TRUE;
  }
...
static void update_is_resumable(struct DCC *dcc) {
  gchar *filename_fs =
      g_filename_from_utf8(dcc->destfile, -1, NULL, NULL, NULL);
  ...
  if (filename_fs != NULL && g_access(filename_fs, W_OK) == 0) {
    ...
    if (file != NULL) {
      ...
      if (file_info != NULL) {
        ...
        guint64 file_size =
            (file_size_offset >= 0) ? (guint64)file_size_offset : 0;
...
void dcc_get(struct DCC *dcc) {
  ...
  case STAT_QUEUED:
    if (dcc->type != TYPE_CHATSEND) {
      if (dcc->type == TYPE_RECV && prefs.hex_dcc_auto_resume &&
          dcc->resumable) {
        dcc_resume(dcc);
      } else {
        dcc->resumable = 0;
...
void dcc_get_with_destfile(struct DCC *dcc, char *file) {
  g_free(dcc->destfile);
...
void dcc_chat(struct session *sess, char *nick, int passive) {
  char outbuf[512];
  struct DCC *dcc;
  ...
  if (passive || dcc_listen_init(dcc, sess)) {
    ...
    if (passive) {
      dcc->pasvid = new_id();
      g_snprintf(outbuf, sizeof(outbuf), "DCC CHAT chat 199 %d %d", dcc->port,
                 dcc->pasvid);
    } else {
      g_snprintf(outbuf, sizeof(outbuf), "DCC CHAT chat %u %d", dcc->addr,
                 dcc->port);
    }
...
int dcc_resume(struct DCC *dcc) {
  ...
  if (dcc->dccstat == STAT_QUEUED && dcc->resumable) {
    ...
    g_snprintf(tbuf, sizeof(tbuf) - 10,
               strchr(dcc->file, ' ')
...
static void dcc_confirm_send(void *ud) {
  struct DCC *dcc = (struct DCC *)ud;
  dcc_get(dcc);
}

static void dcc_deny_send(void *ud) {
  struct DCC *dcc = (struct DCC *)ud;
  dcc_abort(dcc->serv->front_session, dcc);
}

static void dcc_confirm_chat(void *ud) {
  struct DCC *dcc = (struct DCC *)ud;
  dcc_connect(dcc);
}

static void dcc_deny_chat(void *ud) {
  struct DCC *dcc = (struct DCC *)ud;
  dcc_abort(dcc->serv->front_session, dcc);
}

static struct DCC *dcc_add_chat(session *sess, char *nick, int port,
                                guint32 addr, int pasvid) {
  struct DCC *dcc;
  ...
  if (dcc) {
    ...
    EMIT_SIGNAL(XP_TE_DCCCHATOFFER, sess->server->front_session, nick, NULL,
                NULL, NULL, 0);
...
static struct DCC *dcc_add_file(session *sess, char *file, guint64 size,
                                int port, char *nick, guint32 addr,
                                int pasvid) {
  struct DCC *dcc;
  char tbuf[512];
  ...
  if (dcc) {
    ...
    dcc->destfile =
        g_malloc(strlen(prefs.hex_dcc_dir) + strlen(nick) + strlen(file) + 4);
  ...
  EMIT_SIGNAL(XP_TE_DCCSENDOFFER, sess->server->front_session, nick, file, tbuf,
              tbuf + 24, 0);

  return dcc;
}

void handle_dcc(struct session *sess, char *nick, char *word[],
...
void dcc_show_list(struct session *sess) {
  int i = 0;
  struct DCC *dcc;
  GSList *list = dcc_list;
  ...
  while (list) {
    ...
    i++;
    PrintTextf(sess,
               " %s  %-10.10s %-7.7s %-7" G_GUINT64_FORMAT
               " %-7" G_GUINT64_FORMAT " %s\n",
               dcctypes[dcc->type], dcc->nick, _(dccstat[dcc->dccstat].name),
               dcc->size, dcc->pos, file_part(dcc->file));
    list = list->next;
  }
  if (!i)
