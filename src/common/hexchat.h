...
struct hexchatprefs {
  ...
  unsigned int
      wait_on_exit; /* wait for logs to be flushed to disk IF we're connected */
...
typedef struct server {
  ...
  char *encoding;
  GIConv read_converter;  /* iconv converter for converting from server encoding
                             to UTF-8. */
  GIConv write_converter; /* iconv converter for converting from UTF-8 to server
                             encoding. */
  ...
  unsigned int have_msgid : 1;            /* cap msgid */
  unsigned int have_labeled_response : 1; /* cap labeled-response */
  unsigned int have_message_tags : 1;     /* cap message-tags */
  unsigned int have_chghost : 1;          /* cap chghost */
  unsigned int have_setname : 1;          /* cap setname */
  ...
  unsigned int accept_invalid_cert
      : 1;                      /* ignore result of server's cert. verify */
...
/* CL: get a random int in the range [0..n-1]. DON'T use rand() % n, it gives
 * terrible results. */
