...
#include "url.h"
#include "util.h"
static void irc_login(server *serv, char *user, char *realname) {
  tcp_sendf(
      serv,
...
static void irc_nickserv(server *serv, char *cmd, char *arg1, char *arg2,
                         char *arg3) {
  ...
  case LOGIN_NICKSERV:
    ...
    break;
  default: /* This may not work but at least it tries something when using /id
              or /ghost cmd */
    tcp_sendf(serv, "NICKSERV %s %s%s%s\r\n", cmd, arg1, arg2, arg3);
    break;
#if 0
...
static void irc_ns_identify(server *serv, char *pass) {
  switch (serv->loginmethod) {
...
static void irc_ns_ghost(server *serv, char *usname, char *pass) {
  if (serv->loginmethod != LOGIN_CHALLENGEAUTH) {
    irc_nickserv(serv, "GHOST", usname, " ", pass);
  }
}

static void irc_join(server *serv, char *channel, char *key) {
  if (key[0])
...
static void irc_join_list_flush(server *serv, GString *channels, GString *keys,
                                int send_keys) {
  char *chanstr;
  char *keystr;

  chanstr =
      g_string_free(channels, FALSE); /* convert our strings to char arrays */
  keystr = g_string_free(keys, FALSE);

  if (send_keys) {
    tcp_sendf(serv, "JOIN %s %s\r\n", chanstr,
              keystr); /* send the actual command */
  } else {
...
static void irc_join_list(server *serv, GSList *favorites) {
  int first_item = 1; /* determine whether we add commas or not */
  int send_keys = 0;  /* if none of our channels have keys, we can omit the 'x'
                         fillers altogether */
  ...
  while (favlist) {
    ...
    if (len >= 512) /* command length exceeds the IRC hard limit, flush it and
                       start from scratch */
    {
    ...
    if (fav->key) {
      ...
      g_string_append_c(keylist,
                        'x'); /* 'x' filler for keyless channels so that our
                                 JOIN command is always well-formatted */
    }
...
static void irc_part(server *serv, char *channel, char *reason) {
  if (reason[0])
...
static void irc_quit(server *serv, char *reason) {
  if (reason[0])
...
static void irc_set_away(server *serv, char *reason) {
  if (reason) {
    if (!reason[0])
      reason = " ";
  } else {
...
static void irc_ctcp(server *serv, char *to, char *msg) {
  tcp_sendf(serv, "PRIVMSG %s :\001%s\001\r\n", to, msg);
}

static void irc_nctcp(server *serv, char *to, char *msg) {
  tcp_sendf(serv, "NOTICE %s :\001%s\001\r\n", to, msg);
}

static void irc_cycle(server *serv, char *channel, char *key) {
  tcp_sendf(serv, "PART %s\r\nJOIN %s %s\r\n", channel, channel, key);
}

static void irc_kick(server *serv, char *channel, char *nick, char *reason) {
  if (reason[0])
...
static void irc_invite(server *serv, char *channel, char *nick) {
  tcp_sendf(serv, "INVITE %s %s\r\n", nick, channel);
}

static void irc_mode(server *serv, char *target, char *mode) {
  tcp_sendf(serv, "MODE %s %s\r\n", target, mode);
}

/* find channel info when joined */

static void irc_join_info(server *serv, char *channel) {
  tcp_sendf(serv, "MODE %s\r\n", channel);
}

/* initiate userlist retreival */

static void irc_user_list(server *serv, char *channel) {
  if (serv->have_whox)
...
static void irc_userhost(server *serv, char *nick) {
  tcp_sendf(serv, "USERHOST %s\r\n", nick);
}

static void irc_away_status(server *serv, char *channel) {
  if (serv->have_whox)
    tcp_sendf(serv, "WHO %s %%chtsunfra,152\r\n", channel);
  else
    tcp_sendf(serv, "WHO %s\r\n", channel);
 ...
 */
static void irc_user_whois(server *serv, char *nicks) {
  tcp_sendf(serv, "WHOIS %s\r\n", nicks);
}

static void irc_message(server *serv, char *channel, char *text) {
  tcp_sendf(serv, "PRIVMSG %s :%s\r\n", channel, text);
}

static void irc_action(server *serv, char *channel, char *act) {
  tcp_sendf(serv, "PRIVMSG %s :\001ACTION %s\001\r\n", channel, act);
}

static void irc_notice(server *serv, char *channel, char *text) {
  tcp_sendf(serv, "NOTICE %s :%s\r\n", channel, text);
}

static void irc_topic(server *serv, char *channel, char *topic) {
  if (!topic)
...
static void irc_list_channels(server *serv, char *arg, int min_users) {
  if (arg[0]) {
...
static void irc_names(server *serv, char *channel) {
  tcp_sendf(serv, "NAMES %s\r\n", channel);
}

static void irc_change_nick(server *serv, char *new_nick) {
  tcp_sendf(serv, "NICK %s\r\n", new_nick);
}

static void irc_ping(server *serv, char *to, char *timestring) {
  if (*to)
...
static int irc_raw(server *serv, char *raw) {
  int len;
  char tbuf[4096];
  if (*raw) {
    len = strlen(raw);
...
static void channel_date(session *sess, char *chan, char *timestr,
...
static int trailing_index(char *word_eol[]) {
  int param_index;
...
static void process_numeric(session *sess, int n, char *word[],
                            char *word_eol[], char *text,
                            const message_tags_data *tags_data) {
  ...
  case 311: /* WHOIS 1st line */
    ...
    if (!serv->skip_next_whois)
      EMIT_SIGNAL_TIMESTAMP(XP_TE_WHOIS1, whois_sess, word[4], word[5], word[6],
                            (word_eol[8][0] == ':') ? word_eol[8] + 1
                                                    : word_eol[8],
  ...
  case 314: /* WHOWAS */
    inbound_user_info_start(sess, word[4], tags_data);
    EMIT_SIGNAL_TIMESTAMP(XP_TE_WHOIS1, whois_sess, word[4], word[5], word[6],
                          word_eol[8] + 1, 0, tags_data->timestamp);
    break;
  ...
  case 317:
    if (!serv->skip_next_whois) {
      ...
      g_snprintf(outbuf, sizeof(outbuf), "%02ld:%02ld:%02ld", idle / 3600,
                 (idle / 60) % 60, idle % 60);
      if (timestamp == 0)
        EMIT_SIGNAL_TIMESTAMP(XP_TE_WHOIS4, whois_sess, word[4], outbuf, NULL,
                              NULL, 0, tags_data->timestamp);
      else {
        tim = ctime(&timestamp);
        if (tim != NULL)
          tim[19] = 0; /* get rid of the \n */
        EMIT_SIGNAL_TIMESTAMP(XP_TE_WHOIS4T, whois_sess, word[4], outbuf, tim,
                              NULL, 0, tags_data->timestamp);
      }
    }
    break;
  ...
  case 318: /* END OF WHOIS */
    if (!serv->skip_next_whois)
      EMIT_SIGNAL_TIMESTAMP(XP_TE_WHOIS6, whois_sess, word[4], NULL, NULL, NULL,
                            0, tags_data->timestamp);
  ...
  case 324:
    ...
    if (sess->ignore_mode)
      ...
      EMIT_SIGNAL_TIMESTAMP(XP_TE_CHANMODES, sess, word[4],
                            (word_eol[5][0] == ':') ? word_eol[5] + 1
                                                    : word_eol[5],
                            NULL, NULL, 0, tags_data->timestamp);
  ...
  case 328: /* channel url */
    sess = find_channel(serv, word[4]);
    if (sess) {
      EMIT_SIGNAL_TIMESTAMP(XP_TE_CHANURL, sess, word[4],
                            (word_eol[5][0] == ':') ? word_eol[5] + 1
                                                    : word_eol[5],
                            NULL, NULL, 0, tags_data->timestamp);
    }
  ...
  case 329:
    ...
    if (sess) {
      if (sess->ignore_date)
        ...
        channel_date(sess, word[4], (word[5][0] == ':') ? word[5] + 1 : word[5],
                     tags_data);
    }
    break;
  ...
  case 333:
    inbound_topictime(serv, word[4], word[5],
                      atol(STRIP_COLON(word, word_eol, 6)), tags_data);
    break;
  ...
  case 341: /* INVITE ACK */
    EMIT_SIGNAL_TIMESTAMP(XP_TE_UINVITE, sess, word[4],
                          STRIP_COLON(word, word_eol, 5), serv->servername,
                          NULL, 0, tags_data->timestamp);
    break;
  ...
  case 352: /* WHO */
  {
    ...
    inbound_user_info(sess, word[4], word[5], word[6], word[7], word[8],
                      word_eol[11], NULL, away, tags_data);
  ...
  case 354: /* undernet WHOX: used as a reply for irc_away_status */
  {
    ...
    if (!strcmp(word[4], "152")) {
      ...
      /* :server 354 yournick 152 #channel ~ident host servname nick H account
       * :realname */
      inbound_user_info(sess, word[5], word[6], word[7], word[8], word[9],
                        word_eol[12] + 1, word[11], away, tags_data);
  ...
  case 346: /* +I-list entry */
    if (!inbound_banlist(sess, atol(STRIP_COLON(word, word_eol, 7)), word[4],
                         word[5], word[6], 346, tags_data))
      goto def;
    break;
  ...
  case 706: // RPL_ENDOFHELP
    EMIT_SIGNAL_TIMESTAMP(XP_TE_SERVTEXT, sess, STRIP_COLON(word, word_eol, 5),
                          NULL, NULL, NULL, 0, tags_data->timestamp);
    break;
  ...
  case 900: /* successful SASL 'logged in as ' */
    EMIT_SIGNAL_TIMESTAMP(XP_TE_SERVTEXT, serv->server_session, word_eol[6] + 1,
                          word[1], word[2], NULL, 0, tags_data->timestamp);
    break;
    ...
    if (is_channel(serv, word[4])) {
      ...
      if (!sess)
        sess = serv->server_session;
                    ...
                    find_dialog(serv, word[4]))) /* user with an open dialog */
      ;
    else
      sess = serv->server_session;

    EMIT_SIGNAL_TIMESTAMP(XP_TE_SERVTEXT, sess, text, word[1], word[2], NULL, 0,
                          tags_data->timestamp);
  }
  }
}
...
static void process_named_msg(session *sess, char *type, char *word[],
                              char *word_eol[],
                              const message_tags_data *tags_data) {
  server *serv = sess->server;
  char *account;
  ...
  if (serv->have_account_tag) {
    account =
        tags_data->account && *tags_data->account ? tags_data->account : "*";
  ...
  if (len == 4) {
    ...
    case WORDL('K', 'I', 'L', 'L'): {
      ...
      EMIT_SIGNAL_TIMESTAMP(XP_TE_KILL, sess, nick, reason, NULL, NULL, 0,
                            tags_data->timestamp);
    }
      return;
    ...
    case WORDL('F', 'A', 'I', 'L'):
      ...
      if (g_strcmp0(word[3], "*") == 0) {
        EMIT_SIGNAL_TIMESTAMP(XP_TE_FAIL, sess, word[4], text, NULL, NULL, NULL,
                              tags_data->timestamp);
      } else {
        EMIT_SIGNAL_TIMESTAMP(XP_TE_FAILCMD, sess, word[3], word[4], text, NULL,
    ...
    case WORDL('W', 'A', 'R', 'N'):
      text = STRIP_COLON(word, word_eol, trailing_index(word_eol));
      if (g_strcmp0(word[3], "*") == 0) {
        EMIT_SIGNAL_TIMESTAMP(XP_TE_WARN, sess, word[4], text, NULL, NULL, NULL,
                              tags_data->timestamp);
      } else {
        EMIT_SIGNAL_TIMESTAMP(XP_TE_WARNCMD, sess, word[3], word[4], text, NULL,
                              NULL, tags_data->timestamp);
      }
      return;
    ...
    case WORDL('N', 'O', 'T', 'E'):
      text = STRIP_COLON(word, word_eol, trailing_index(word_eol));
      if (g_strcmp0(word[3], "*") == 0) {
        EMIT_SIGNAL_TIMESTAMP(XP_TE_NOTE, sess, word[4], text, NULL, NULL, NULL,
                              tags_data->timestamp);
      } else {
        EMIT_SIGNAL_TIMESTAMP(XP_TE_NOTECMD, sess, word[3], word[4], text, NULL,
                              NULL, tags_data->timestamp);
      }
      return;
    }
    ...
    case WORDL('C', 'H', 'G', 'H'):
      inbound_user_info(sess, NULL, word[3], STRIP_COLON(word, word_eol, 4),
                        NULL, nick, NULL, NULL, 0xff, tags_data);
      return;

    case WORDL('S', 'E', 'T', 'N'):
      inbound_user_info(sess, NULL, NULL, NULL, NULL, nick,
                        STRIP_COLON(word, word_eol, 3), NULL, 0xff, tags_data);
      return;
    ...
    case WORDL('I', 'N', 'V', 'I'):
      ...
      if (serv->p_cmp(word[3], serv->nick))
        EMIT_SIGNAL_TIMESTAMP(XP_TE_INVITEDOTHER, sess, text, nick, word[3],
                              serv->servername, 0, tags_data->timestamp);
      else
        EMIT_SIGNAL_TIMESTAMP(XP_TE_INVITED, sess, text, nick, serv->servername,
                              NULL, 0, tags_data->timestamp);
    ...
    case WORDL('N', 'O', 'T', 'I'): {
      ...
      if (serv->loginmethod == LOGIN_CHALLENGEAUTH &&
          !serv->p_cmp(word[1], CHALLENGEAUTH_FULLHOST) &&
      ...
      if (!ignore_check(word[1], IG_NOTI))
        inbound_notice(serv, word[3], nick, text, ip, tags_data->identified,
                       tags_data);
    }
      return;
      ...
      if (*to) {
        ...
        if (text[0] == 1) /* ctcp */
        {
          ...
          if (g_ascii_strncasecmp(text, "DCC ", 4) == 0) {
            ...
            /* This is a bit ugly but we handle the contents of the DCC message
             * containing "quoted paths for files" here which means reparsing
             * the message with handle_quotes. We avoid reparsing the entire
             * message to avoid corrupting the non DCC parts. Greater than
             * PDIWORD length DCC messages will be truncated. */
            process_data_init(new_pdibuf, word_eol[6], new_word, new_word_eol,
                              TRUE, FALSE);
          ...
          ctcp_handle(sess, to, nick, ip, text, word, word_eol,
          ...
          if (is_channel(serv, to)) {
            ...
            if (ignore_check(word[1], IG_PRIV))
              return;
            inbound_privmsg(serv, nick, ip, text, tags_data->identified,
                            tags_data);
          }
        }
      }
    ...
    case WORDL('C', 'A', 'P', '\0'):
      if (strncasecmp(word[4], "ACK", 3) == 0) {
        inbound_cap_ack(serv, word[1],
                 ...
                 strncasecmp(word[4], "NEW", 3) == 0) {
        inbound_cap_ls(serv, word[1],
                       word[5][0] == ':' ? word_eol[5] + 1 : word_eol[5],
                       tags_data);
      } else if (strncasecmp(word[4], "NAK", 3) == 0) {
        inbound_cap_nak(serv, word[5][0] == ':' ? word_eol[5] + 1 : word_eol[5],
                        tags_data);
...
static void process_named_servermsg(session *sess, char *buf, char *rawname,
                                    char *word_eol[],
                                    const message_tags_data *tags_data) {
  ...
  if (!strncmp(buf, "ERROR", 5)) {
    EMIT_SIGNAL_TIMESTAMP(XP_TE_SERVERERROR, sess, buf + 7, NULL, NULL, NULL, 0,
                          tags_data->timestamp);
    return;
  }
  if (!strncmp(buf, "NOTICE ", 7)) {
    ...
    if (*buf == ':')
      buf++;
    EMIT_SIGNAL_TIMESTAMP(XP_TE_SERVNOTICE, sess, buf, sess->server->servername,
                          NULL, NULL, 0, tags_data->timestamp);
    return;
  }
 ...
 */
static void handle_message_tag_time(const char *time,
 ...
 */
static void handle_message_tags(server *serv, const char *tags_str,
                                message_tags_data *tags_data) {
  char **tags;
  int i;
